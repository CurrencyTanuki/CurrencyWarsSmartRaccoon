...
# 2026-08-22：WiX 4.0.6 便携生成器与便携 dotnet 位于 Codex 主工作区 .tools，
# 通过绝对路径引用（CWAFix-20260814 仓库树内不重复存放 .tools）。
$dotnet = Join-Path $repositoryRoot '.tools\dotnet\dotnet.exe'
$wix = Join-Path $repositoryRoot '.tools\wix\wix.exe'
$codexTools = 'D:\Codex-2\.tools'
if (-not (Test-Path -LiteralPath $dotnet)) {
    $dotnet = (Join-Path $codexTools 'dotnet\dotnet.exe')
}
if (-not (Test-Path -LiteralPath $wix)) {
    $wix = (Join-Path $codexTools 'wix\wix.exe')
}
