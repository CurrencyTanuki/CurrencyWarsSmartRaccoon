...
## 〇-12、09-10 深夜：逐帧排查三根因一并修复（e35e689，**待子代理对抗审查+未发布**）

- **①商店识别稳定门**（9540c51，详见〇-11）：观测上限 2→3+单帧 miss 容忍——帧1 动画乱码不再丢弃帧2 正确读数（吉尔伽美什+Saber 实锤修复）。回归测试 5 例。
- **②金币/血量捕获时间戳=帧龄**（GrailSnapshotAssembler.cs:112+GrailRecognitionCommands.cs ReadMeter）：`CaptureGold/CaptureHealth` 改用分析帧 `Snapshot.AsOf`（无帧兜底墙钟），滞后管线帧不再以"新钟"伪装新鲜压过本地账——终态金 1→32→1 幻想根因闭合（I1 同族 09-02 已修，本次对齐 I5/I6/Assemble）。**附带收益**：R3 判死条件（金<刷新价）不再被滞后高读数 defeating——18:08-18:11 的 3 分钟金尽拖延同根因消除。
- **③徽章拖拽强化**：按压 450→700ms、时长 850→1100ms（1.2.121 会话 450 档在 15:58-17:09 负载时段 18 连败实证；时段性吸附劣化，字段对比零差异）。
- **验证**：构建 0/0+Grail/Shop/Reward 套件 297/297+识别门回归 42/42。**待办**：子代理对抗审查（本批 3 commit：9540c51/e35e689/前批）；未发布；18:14:28 金=13 卖卡芙卡违规存疑（需 UI 日志）；决策层 emit 落 jsonl（事件分类设计单独批）；84 秒空窗前扩抽帧。
