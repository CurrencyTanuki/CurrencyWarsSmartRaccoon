...
public partial class GameAreaCalibrationWindow : Window
{
    ...
    /// <summary>
    /// 预览区最大高度：优先保证按钮行可见——窗口总高度（标题 + 预览 +
    /// 底部按钮行）不得超过工作区高度，否则高 DPI / 小屏幕下
    /// "确认游戏区域"按钮会被挤出窗口可视区。
    /// </summary>
    private double MaximumPreviewHeight
    {
        get
        {
            ...
            const double verticalChrome =
                ...
                40 /* 底部按钮行 */ +
                32 /* Grid Margin */ +
                28 /* 预览 Border 上下 Margin */ +
                30 /* 窗口标题栏 */;
            var available = workAreaHeight - verticalChrome;
            return Math.Clamp(available, 320, 610);
        }
    }
    ...
    {
        // 云游戏浏览器切前台会从全屏还原/缩小（窗口乱动），
        // 重新截图由 GdiGameCapture 的 PrintWindow 回退兜底。
        if (_hostWindow.SourceKind != GameWindowSourceKind.CloudBrowser)
        {
