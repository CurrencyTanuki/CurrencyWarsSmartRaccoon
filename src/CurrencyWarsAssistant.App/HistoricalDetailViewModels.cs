...
internal sealed class HistoricalDetailPresentationBuilder(GameDataCatalog catalog)
{
    ...
    private readonly IReadOnlyDictionary<string, IReadOnlyList<int>> characterCosts =
        catalog.CurrencyWarsCharacters.ToDictionary(
            item => item.Id,
            item => item.Costs,
            StringComparer.OrdinalIgnoreCase);
        ...
        IReadOnlyList<CompletedRunNodeRecord> nodes)
    {
        ...
        for (var index = 0; index < nodes.Count; index++)
        {
            ...
            var action = battle is null
                ? KnownValue(snapshot?.ActionPoints)
            ...
            var damageDisplay = damage is null
                ...
                : damage.Value >= 1_000_000
                    ...
                    : $"{damage.Value:N0}";
            var theoryDisplay = TheoreticalDamageCalculator.HasActionLimit(
                node.NodeId)
                ? battle?.TheoreticalDamageLimit is { } theory
                    ? FormatDamage(theory)
                    : "未记录"
                : "不适用（该节点没有行动值上限）";
        ...
        FinalNodeBattleState? battle)
    {
        var rows = new List<HistoricalDetailFieldRow>
        {
            ...
            new("节点类型", battle is null
                ? "数据不足"
                : IsRewardNodeId(battle.NodeId) ? "奖励关" : "战斗节点",
        ...
        HistoricalDetailEconomyProjection? economy)
    {
        ...
        Add(rows, "连胜数", state?.CumulativeSpend ?? snapshot?.CumulativeSpend);
        ...
        Phase2OperationalState? state)
    {
        ...
        if (formation?.Value is { Count: > 0 } characters)
        {
            ...
            {
                characterCosts.TryGetValue(
                    character.CharacterId,
                    out var catalogCosts);
                var displayCost = CharacterCostDisplay.Resolve(
                    character.CurrentCost,
                    catalogCosts);
                ...
                rows.Add(new HistoricalDetailFieldRow(
                    ...
                    $"费用：{(displayCost.HasValue ? $"{displayCost}费" : "未记录")} · " +
                    $"装备：{equipment}",
                ...
                for (var slot = 0; slot < 3; slot++)
                {
                    ...
                    var privilegeSuffix = slotState?.IsPrivileged == true
                        ? " · 特权"
                        : string.Empty;
                    var value = slotState?.Occupancy switch
                    {
                        ...
                        EquipmentSlotOccupancy.Equipped =>
                            (equipmentId ?? "未知装备") + privilegeSuffix,
                        _ => equipmentId ?? "未记录"
                    };
                    rows.Add(new HistoricalDetailFieldRow(
        ...
        Phase2OperationalState? state)
    {
        ...
        AddSpecialItemRows(rows, snapshot?.SpecialItemIds);
        var inventory = state?.InventorySlots ?? snapshot?.InventorySlots;
    ...
    private static void AddSpecialItemRows(
        ICollection<HistoricalDetailFieldRow> rows,
        Observation<IReadOnlyList<string>>? observation)
    {
        if (observation?.Value is not { Count: > 0 } itemIds)
        {
            rows.Add(new HistoricalDetailFieldRow(
                "特殊物品",
                "未记录",
                observation is null
                    ? "原因=节点没有特殊物品状态"
                    : $"状态={observation.Status}；原因={ListValue(observation.Uncertainty)}"));
            return;
        }

        for (var index = 0; index < itemIds.Count; index++)
        {
            var iconPath = ResolveIconPath("special", itemIds[index]);
            rows.Add(new HistoricalDetailFieldRow(
                $"特殊物品 {index + 1}",
                string.Empty,
                iconPath is null ? "图片资源缺失" : "已记录",
                iconPath));
        }
    }
        ...
        Phase2OperationalState? state)
    {
        ...
        if (state?.ActiveSynergies.Value is { Count: > 0 } synergies)
        {
            ...
            {
                var nextText = synergy.NextThreshold is { } next
                    ? next.ToString()
                    : "已满级";
                rows.Add(new HistoricalDetailFieldRow(
                    $"羁绊 {synergy.SlotKey}",
        ...
        FinalNodeBattleState? battle)
    {
        ...
        var hasActionLimit = TheoreticalDamageCalculator.HasActionLimit(
            battle.NodeId);
        const string notApplicable = "不适用（该节点没有行动值上限）";
        ...
        [
            ...
            new("节点类型", IsRewardNodeId(battle.NodeId) ? "奖励关" : "战斗节点"),
            ...
            new("血量变化",
                battle.HealthDepleted
                    ? "下降（具体数值未知）"
                    : healthDelta.Uncertainty is null
                        ? Signed(healthDelta.Value)
                        : "未知（异常增量已拒绝）",
                healthDelta.Uncertainty ?? string.Empty),
            ...
            new("理论出伤极限", hasActionLimit
                ? FormatDamage(battle.TheoreticalDamageLimit)
                : notApplicable),
            new("理论值规则", hasActionLimit
                ? battle.TheoreticalDamageRule ?? "未记录"
                : notApplicable),
            new("理论值质量", hasActionLimit
                ? battle.TheoreticalDamageQuality.ToString()
                : "不适用"),
            new("基础/有效最大行动", hasActionLimit
                ? $"{battle.BaseMaximumActionValue?.ToString() ?? "?"} / " +
                  $"{battle.EffectiveMaximumActionValue?.ToString() ?? "?"}"
                : "不适用 / 不适用"),
            new("确认增加行动", hasActionLimit
                ? battle.ConfirmedActionIncrease?.ToString() ?? "未记录"
                : "不适用"),
            new("奖励关", IsRewardNodeId(battle.NodeId) ? "是" : "否"),
    ...
    private static string? ResolveIconPath(string? category, string? id)
    {
        ...
        var folder = category switch
        {
            ...
            "special" when id.StartsWith(
                "currency_wars_equipment_",
                StringComparison.OrdinalIgnoreCase) => Path.Combine(
                    "raw", "4.4", "equipment", "890ae486642e979b", "assets",
                    "currency_wars_equipment_icons"),
            "special" => Path.Combine(
                "4.4", "phase2-icon-assets", "standardized", "special_item"),
            _ => category
        };
    ...
    /// <summary>
    /// 奖励关判定（用户 2026-08-06 确认）：1-1、1-2、1-8、2-6、3-6 共 5 个。
    /// 按节点号硬编码，不依赖页面识别（IsRewardNode 字段不可靠）。
    /// </summary>
