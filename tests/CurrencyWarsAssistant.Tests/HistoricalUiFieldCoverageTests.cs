...
public sealed class HistoricalUiFieldCoverageTests
{
    ...
    public void DetailBuilder_ShowsEveryCharacterEquipmentSlotWithoutInventingEmptySlots()
    {
        ...
        Assert.Contains(formation.Fields, row =>
            row.Label == "前台 1" && row.Value.Contains("费用：1费"));
        Assert.Contains(formation.Fields, row =>
    ...
    public void DetailBuilder_ProjectsInventoryEvidenceArchiveFilesAndAffixSlots()
    {
        ...
        var specialItem = Assert.Single(rows.Where(row =>
            row.Label == "特殊物品 1"));
        Assert.Equal(string.Empty, specialItem.Value);
        Assert.NotNull(specialItem.IconPath);
        Assert.Contains(
            Path.Combine(
                "raw", "4.4", "equipment", "890ae486642e979b", "assets",
                "currency_wars_equipment_icons",
                "currency_wars_equipment_153.png"),
            specialItem.IconPath,
