...
- **1.2.85（防接手旧局守卫，2026-09-05 凌晨，行为审计任务二 TOP2）**：M8 快速路径盲点 (1690,967)×4s 落在 rank_difficulty_in_progress 对话框按钮区（0.58-0.99/0.82-0.97）内——上局残留弹该对话框时盲点会误点"继续进度"绕过 IgnoreActiveRun 保护接手用户旧局。修复=新增 BlindClickStartRunWithGuardAsync（每 2 秒单帧判定，命中对话框立即停手返回失败交上层按旧局处理），替代原裸 BlindClickAsync 调用。构建 0/0、Grail 111。结算推进 12 连点页门禁（任务二 TOP3）列入下批（现版弃局链每击已含页面识别，实测工作正常）。
