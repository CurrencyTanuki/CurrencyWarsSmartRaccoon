...
public sealed class RunCompletionArchiveTests
{
    ...
    public async Task CompletionArchiveIsAtomicAndIdempotent()
    {
        ...
        try
        {
            ...
            // 问题3回归防护：sha256 必须与实际文件字节匹配（无 BOM 写入），
            // 否则会被误判为"写入中断/归档损坏"（实测 run-20260805-005633）。
            var archiveBytes = await File.ReadAllBytesAsync(Path.Combine(
                store.GetRunDirectory(runId),
                "completed-run.v1.json"));
            var recordedSha = (await File.ReadAllTextAsync(Path.Combine(
                    store.GetRunDirectory(runId),
                    "completed-run.v1.json.sha256")))
                .Trim();
            var actualSha = Convert.ToHexString(
                System.Security.Cryptography.SHA256.HashData(archiveBytes));
            Assert.Equal(recordedSha, actualSha);
