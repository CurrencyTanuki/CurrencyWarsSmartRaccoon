...
public partial class CompletedRunsWindow : Window
{
    ...
    internal async Task RenderReportAsync(string runId)
    {
        ...
        try
        {
            ...
            // 弹性初始化：复用损坏/跨内核版本不兼容的旧 user data 目录失败时
            // 自动改名重建（2026-08-10 修复 0x800700AA）。
            if (ReportBrowser.CoreWebView2 is null)
            {
                var (initialized, webViewFailure) = await WebView2Initializer.EnsureInitializedAsync(
                    ReportBrowser);
                var status = initialized ? "成功" : "失败";
                File.AppendAllText(logPath,
                    $"{DateTime.Now:HH:mm:ss} WebView2 已初始化: {status}" + Environment.NewLine);
                if (!initialized)
                {
            ...
            else
            {
                ReportLoading.Visibility = Visibility.Collapsed;
                ReportBrowser.NavigateToString(
                    "<p style='color:#e06060;font-family:Microsoft YaHei UI;padding:24px'>报告生成失败（内置报告组件缺失或损坏）</p>");
            }
        }
        catch (Exception exception)
        {
            ...
            ReportLoading.Visibility = Visibility.Collapsed;
            if (ReportBrowser.CoreWebView2 is not null)
            {
                ReportBrowser.NavigateToString(
