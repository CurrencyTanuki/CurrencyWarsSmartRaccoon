...
""");
        ...
        ChallengeReportDocument document)
    {
        var bestDamage = document.Nodes
            .Where(item => item.FinalDamage.HasValue && !IsRewardNodeId(item.NodeId))
        ...
        ChallengeReportDocument document)
    {
        ...
        AppendChart(builder, "最终伤害", document.Nodes
            .Select(item => new ChartPoint(item.NodeId, IsRewardNodeId(item.NodeId) ? null : item.FinalDamage))
            .ToArray(), "#f3c65f", allowLogarithmic: true, "奖励关不进入伤害趋势");
        AppendChart(builder, "理论出伤极限", document.Nodes
            .Select(item => new ChartPoint(
                item.NodeId,
                TheoreticalDamageCalculator.HasActionLimit(item.NodeId)
                    ? item.Battle?.TheoreticalDamageLimit : null))
        ...
        ChallengeReportFixedFacts facts)
    {
        builder.Append("<h2>对局固定构筑</h2><section class=\"fixed-grid\"><article class=\"panel fixed\"><dl><dt>投资环境</dt><dd>")
            ...
            .Append(H(ListOrUnknown(facts.SpecialItems))).Append("</dd><dt>说明</dt><dd class=\"muted\">这些对局级字段只取最终封存中的有效观测；未知不会被解释为未获得。</dd></dl></article></section>");
    }
        ...
        IReadOnlyList<ChallengeReportNode> nodes)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            foreach (var character in formation.OrderBy(item => item.Zone).ThenBy(item => item.SlotIndex))
            {
                ...
                var displayCost = modelBuilder.CharacterDisplayCost(character);
                ...
                builder.Append("<b>").Append(H(displayName)).Append("</b><small>")
                    ...
                    .Append(displayCost.HasValue ? $" · {displayCost}费" : string.Empty)
        ...
        ChallengeReportNode node)
    {
        ...
        var hasActionLimit = TheoreticalDamageCalculator.HasActionLimit(
            node.NodeId);
        builder.Append("<div class=\"node-extra\">");
        AppendSmallField(builder, "阵容理论出伤极限", hasActionLimit
            ? FormatDamage(node.Battle?.TheoreticalDamageLimit)
            : "不适用（该节点没有行动值上限）");
        AppendSmallField(builder, "金币", FormatNullable(node.Gold));
        AppendSmallField(builder, "金币变化", FormatSigned(node.GoldDelta));
        AppendSmallField(builder, "连胜数", FormatNullable(node.CumulativeSpend));
        ...
        IReadOnlyList<ChallengeReportNode> nodes)
    {
        ...
        foreach (var node in nodes.Where(item => item.Battle is not null))
        {
            ...
            var hasActionLimit = TheoreticalDamageCalculator.HasActionLimit(
                node.NodeId);
            var healthDelta = BattleHealthDeltaPolicy.Evaluate(
                node.NodeId,
                battle.PreBattleHealth,
                battle.PostBattleHealth,
                battle.HealthDelta);
            var healthText = battle.HealthDepleted
                ? $"{FormatNullable(battle.PreBattleHealth)} → 已耗尽（具体变化未知）"
                : healthDelta.Uncertainty is null
                    ? $"{FormatNullable(battle.PreBattleHealth)} → {FormatNullable(battle.PostBattleHealth)}（{FormatSigned(healthDelta.Value)}）"
                    : $"{FormatNullable(battle.PreBattleHealth)} → {FormatNullable(battle.PostBattleHealth)}（未知；异常增量已拒绝）";
            ...
            builder.Append("<div class=\"node-extra\">");
            AppendSmallField(builder, "战前 → 战后血量", healthText);
            AppendSmallField(builder, "完美通关", ClearName(battle.ClearStatus));
            AppendSmallField(builder, "基础 / 有效最大行动", hasActionLimit
                ? $"{FormatNullable(battle.BaseMaximumActionValue)} / {FormatNullable(battle.EffectiveMaximumActionValue)}"
                : "不适用 / 不适用");
            AppendSmallField(builder, "确认增加行动", hasActionLimit
                ? FormatNullable(battle.ConfirmedActionIncrease)
                : "不适用");
            AppendSmallField(builder, "理论值规则", hasActionLimit
                ? string.IsNullOrWhiteSpace(battle.TheoreticalDamageRule)
                    ? "未记录" : battle.TheoreticalDamageRule
                : "不适用（该节点没有行动值上限）");
            AppendSmallField(builder, "理论值质量", hasActionLimit
                ? battle.TheoreticalDamageQuality.ToString()
                : "不适用");
    ...
    private static void AppendNodeTable(
        ...
        IReadOnlyList<ChallengeReportNode> nodes)
    {
        builder.Append("<h2>节点精确数据</h2><section class=\"panel table-wrap\"><table><thead><tr><th>节点</th><th>最终伤害</th><th>来源</th><th>剩余行动</th><th>金币</th><th>金币Δ</th><th>连胜数</th><th>花费Δ</th><th>奖励</th><th>血量Δ</th><th>完美</th><th>理论极限</th><th>质量</th></tr></thead><tbody>");
        foreach (var node in nodes)
        {
            var battle = node.Battle;
            var hasActionLimit = TheoreticalDamageCalculator.HasActionLimit(
            ...
            var healthDelta = BattleHealthDeltaPolicy.Evaluate(
                ...
                battle?.PostBattleHealth,
                battle?.HealthDelta);
            builder.Append("<tr><td>").Append(H(node.NodeId)).Append(IsRewardNodeId(node.NodeId) ? " <span class=\"chip\">奖励</span>" : string.Empty)
                ...
                .Append("</td><td>").Append(Value(
                    battle?.HealthDepleted == true
                        ? "下降（未知）"
                        : healthDelta.Uncertainty is null
                            ? FormatSigned(healthDelta.Value)
                            : "未知（异常增量已拒绝）",
                    battle?.HealthDepleted == true || healthDelta.Value is not null || healthDelta.Uncertainty is not null))
                .Append("</td><td class=\"").Append(ClearClass(battle?.ClearStatus)).Append("\">")
                .Append(H(ClearName(battle?.ClearStatus))).Append("</td><td>")
                .Append(Value(hasActionLimit ? FormatDamage(battle?.TheoreticalDamageLimit) : "不适用", !hasActionLimit || battle?.TheoreticalDamageLimit is not null))
                .Append("</td><td>").Append(H(hasActionLimit ? QualityName(battle) : "不适用")).Append("</td></tr>");
        }

        builder.Replace("<th>奖励</th>", string.Empty);
    ...
    /// 与操作面板 DashboardTrendChart.IsRewardNodeId、投影
    /// HistoricalDashboardProjection.IsRewardNodeId 保持一致。
    /// 不要用 Battle.IsRewardNode 字段（页面类型语义，会把几乎全部节点
    /// 判为奖励 → 对数自动判定只剩 1 个正数 → 强制线性，伤害图被拉平）。
    /// </summary>
