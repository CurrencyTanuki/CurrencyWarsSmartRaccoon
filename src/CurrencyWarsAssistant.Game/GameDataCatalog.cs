...
public sealed class GameDataCatalog
{
    ...
    /// <summary>
    /// 羁绊目录（含 tier_effects：几人口激活几级，如星核猎手 2/3/4 人）。
    /// Load 时由 GameDataCatalogLoader 填充；未加载时为空列表。
    /// </summary>
    public static IReadOnlyList<BondCatalogData> BondCatalog { get; internal set; } = [];
...
public static class GameDataCatalogLoader
{
    ...
    public static GameDataCatalog Load(string directory)
    {
        ...
        LoadBondCatalog(fullDirectory);

        Validate(catalog);
        return catalog;
    }

    private static IReadOnlyList<BondCatalogData> LoadBondCatalog(
        string directory)
    {
        ...
        var export = JsonSerializer.Deserialize<CurrencyWarsCharacterExport>(
            ...
            JsonOptions);
        var result = export?.BondCatalog ?? [];
        GameDataCatalog.BondCatalog = result;
        return result;
    }
    ...
    private sealed record CurrencyWarsCharacterExport(
        ...
        [property: JsonPropertyName("bond_catalog")]
        IReadOnlyList<BondCatalogData>? BondCatalog = null);
}

public sealed record BondTierEffect(
    [property: JsonPropertyName("required_members")] int RequiredMembers,
    [property: JsonPropertyName("effect")] string Effect);

public sealed record BondCatalogData(
    [property: JsonPropertyName("id")] string Id,
    [property: JsonPropertyName("name")] string Name,
    [property: JsonPropertyName("type")] string? Type,
    [property: JsonPropertyName("tier_effects")] IReadOnlyList<BondTierEffect>? TierEffects = null,
    [property: JsonPropertyName("members")] IReadOnlyList<string>? Members = null);
