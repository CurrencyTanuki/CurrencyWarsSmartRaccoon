...
public sealed class TheoreticalDamageCalculatorTests
{
    ...
    [InlineData("2-6")]
    [InlineData("3-6")]
    public void RewardNodeWithoutActionLimitDoesNotCalculateTheory(
        string nodeId)
    {
        var result = TheoreticalDamageCalculator.Calculate(
            nodeId,
            64_153,
            RemainingActionValueState.Create(1, 1),
            NodeClearStatus.Perfect,
            hasWalter: false,
            walterStarLevel: null,
            confirmedActionIncrease: 0,
        ...
        Assert.False(TheoreticalDamageCalculator.HasActionLimit(nodeId));
        Assert.Null(result.Value);
        Assert.Null(result.BaseMaximumActionValue);
        Assert.Null(result.EffectiveMaximumActionValue);
        Assert.Equal(TheoreticalDamageQuality.Unknown, result.Quality);
        Assert.Contains("no action limit", result.Rule);
    }
