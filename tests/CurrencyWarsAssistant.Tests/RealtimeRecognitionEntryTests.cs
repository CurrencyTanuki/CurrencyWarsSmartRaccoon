...
public sealed class RealtimeRecognitionEntryTests
{
    ...
    [Fact]
    public void AutomaticRerollEnablesOverlayClickThroughBeforeShowingOverlay()
    {
        var main = ReadAppFile("MainViewModel.cs");
        var runFilter = main[main.IndexOf(
            "private async Task RunFilterAsync",
            StringComparison.Ordinal)..];
        var clickThrough = runFilter.IndexOf(
            "IsLogOverlayClickThrough = true;",
            StringComparison.Ordinal);
        var assistanceActivated = runFilter.IndexOf(
            "AssistanceActivated?.Invoke",
            StringComparison.Ordinal);

        Assert.True(clickThrough >= 0);
        Assert.True(clickThrough < assistanceActivated);
    }
