...
public sealed class Phase2RealtimeFrameBufferTests
{
    ...
    [Fact]
    public void PreparationDiagnosticFallbackAcceptsSingleGenericAnchor()
    {
        // 备战页不再依赖 specific 节点模板（用户 2026-08-04 方案）：
        // generic"备战阶段"标题达到阈值即判定备战页。
    ...
    {
        // 备战页不再依赖 specific 节点模板：generic"备战阶段"标题单独判定。
        // 低于 GenericPreparationMinimum(0.35) 的弱分数仍拒绝。
        var inferred = Phase2PageDiagnosticFallback.TryInfer(
        [
            new PageAnchorDiagnostic(
                "preparation_generic",
    ...
    {
        // 空白备战席的 generic 模板分数偏低（0.377），但只要达到
        // GenericPreparationMinimum(0.35) 且战斗伤害页签明确缺失即判定备战页。
        var inferred = Phase2PageDiagnosticFallback.TryInfer(
        [
            new PageAnchorDiagnostic(
                "preparation_generic",
    ...
    [Fact]
    public void BlackScreenRecoveryForcesCriticalFrame()
    {
        // 回归防护（问题1：节点状态更新延后）：游戏开战/加载时全黑过渡帧
        // （实测 1-1 约 2 秒亮度 3~16），fast 分类器无页面可判 → 关键帧断裂。
        // 黑屏结束（亮度从 <20 恢复到 >40）必须强制触发关键帧，否则
        // "战斗开始"会延迟到 fast 分类器碰巧匹配 Battle 为止。
        var selector = new Phase2RealtimeFrameSelector();
        var start = DateTimeOffset.UtcNow;

        // 备战页（亮）
        selector.Observe(
            CreateFrame(120) with { CapturedAt = start },
            true,
            Phase2PageFamily.Preparation,
            new Phase2FastPageObservation(
                true, Phase2PageFamily.Preparation, "preparation_generic"));
        // 黑屏过渡（暗，fast 无页面）
        selector.Observe(
            CreateFrame(5) with { CapturedAt = start.AddMilliseconds(200) },
            true,
            Phase2PageFamily.Unknown,
            Phase2FastPageObservation.None);
        // 黑屏中（仍暗）
        selector.Observe(
            CreateFrame(8) with { CapturedAt = start.AddMilliseconds(350) },
            true,
            Phase2PageFamily.Unknown,
            Phase2FastPageObservation.None);
        // 黑屏结束 → 战斗页恢复（亮）——必须触发关键帧
        var recovery = selector.Observe(
            CreateFrame(120) with { CapturedAt = start.AddMilliseconds(500) },
            true,
            Phase2PageFamily.Battle,
            Phase2FastPageObservation.None);

        Assert.Contains(recovery.FramesToRecognize, item => item.IsCritical);
    }

    [Fact]
    public void StableBrightFramesDoNotTriggerBlackScreenRecovery()
    {
        // 反向验证：正常画面（亮度始终 >40）不得误触发黑屏恢复关键帧。
        var selector = new Phase2RealtimeFrameSelector();
        var start = DateTimeOffset.UtcNow;
        for (var index = 0; index < 5; index++)
        {
            var selection = selector.Observe(
                CreateFrame((byte)(60 + index * 20))
                    with { CapturedAt = start.AddMilliseconds(index * 200) },
                true,
                Phase2PageFamily.Preparation,
                new Phase2FastPageObservation(
                    true, Phase2PageFamily.Preparation, "preparation_generic"));
            if (index > 0)
            {
                ...
                Assert.DoesNotContain(
                    selection.FramesToRecognize, item => item.IsCritical);
            }
        }
    }
    ...
    {
        var selector = new Phase2RealtimeFrameSelector();
        var queue = new Phase2BoundedRecognitionQueue(capacity: 6);
        var start = DateTimeOffset.Parse("2026-08-09T13:31:10.000+08:00");
        ...
        Assert.Equal(3, preparation.FramesToRecognize.Count);
        Assert.All(
            preparation.FramesToRecognize,
            item => Assert.True(item.IsCritical));
        Assert.Equal(
            unclassifiedSettlement.Current.Sequence,
            preparation.FramesToRecognize[^2].BufferedFrame.Sequence);
        Assert.Equal(
            preparation.Current.Sequence,
            preparation.FramesToRecognize[^1].BufferedFrame.Sequence);

        foreach (var selected in preparation.FramesToRecognize)
        {
            Assert.True(queue.Enqueue(Work(
                selected.BufferedFrame,
                $"boundary-{selected.BufferedFrame.Sequence}",
                selected.IsCritical)));
        }

        queue.DropStaleFrames();

        var retainedSettlement = await queue.DequeueAsync(CancellationToken.None);
        Assert.True(retainedSettlement.IsCritical);
        Assert.Equal(
            unclassifiedSettlement.Current.Sequence,
            retainedSettlement.BufferedFrame.Sequence);

        var retainedPreparation = await queue.DequeueAsync(CancellationToken.None);
        Assert.True(retainedPreparation.IsCritical);
        Assert.Equal(
            preparation.Current.Sequence,
            retainedPreparation.BufferedFrame.Sequence);
    ...
    public void StableFiveFpsStreamQueuesAFullRefreshAtTwoSecondCadence()
    {
        ...
        var fulls = selected
            ...
            .ToArray();
        // 2s 全量兜底节律不能被增量帧破坏：全量帧必须仍在 0/2/4/6s。
        Assert.Equal(4, fulls.Length);
        Assert.Equal(start, fulls[0].BufferedFrame.Frame.CapturedAt);
        Assert.Equal(start.AddSeconds(2), fulls[1].BufferedFrame.Frame.CapturedAt);
        Assert.Equal(start.AddSeconds(4), fulls[2].BufferedFrame.Frame.CapturedAt);
        Assert.Equal(start.AddSeconds(6), fulls[3].BufferedFrame.Frame.CapturedAt);
        // 累计：4 全量 + 3 增量（静态帧每 1.5s 一次，落在两次全量之间）。
        Assert.Equal(7, selected.Count);
        Assert.Contains(selected, item => item.IsIncremental);
    }
