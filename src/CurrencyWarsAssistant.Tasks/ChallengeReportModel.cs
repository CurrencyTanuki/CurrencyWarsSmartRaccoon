...
internal sealed class ChallengeReportModelBuilder(GameDataCatalog? catalog = null)
{
    ...
    private readonly IReadOnlyDictionary<string, IReadOnlyList<int>> _characterCosts =
        (catalog?.CurrencyWarsCharacters ?? [])
        .ToDictionary(item => item.Id, item => item.Costs, StringComparer.OrdinalIgnoreCase);
    ...
    public int? CharacterDisplayCost(FormationCharacterState character)
    {
        _characterCosts.TryGetValue(
            character.CharacterId,
            out var catalogCosts);
        return CharacterCostDisplay.Resolve(
    ...
    {
        var trusted = nodes
            .Where(item => item.HasTrustedDamage && !IsRewardNodeId(item.NodeId))
            .ToArray();
        ...
        var actionRatios = trusted
            .Where(item =>
                ...
                item.Battle?.RemainingActionValue is not null)
            .Select(item =>
            {
    ...
    /// 与操作面板 DashboardTrendChart、投影 HistoricalDashboardProjection、
    /// HTML 渲染 ChallengeReportHtmlRenderer 的 IsRewardNodeId 保持一致。
    /// 不要用 Battle.IsRewardNode 字段（页面类型语义 reward_battle/
    /// battle_generic，实机几乎全 True → 可信节点被滤到 <3 → 整局评价
    /// 永远"数据不足"，且 2-6 类奖励关失真伤害污染位面覆盖统计）。
    /// </summary>
