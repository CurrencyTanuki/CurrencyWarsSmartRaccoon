...
public sealed class Phase2OperationalCollectionTests
{
    ...
    [Theory]
    [InlineData("run-171955-preparation-1-3-stable.png", "1-3")]
    [InlineData("preparation-1-4-user-2026-08-01.png", "1-4")]
    [InlineData("preparation-1-7-user.png", "1-7")]
    public async Task PreparationFramesBeyondOneTwoResolveNodeFromFullTextOcr(
        string fileName,
        string expectedNode)
    {
        // 回归防护（用户 2026-08-04 实测：打到 1-6 软件卡 1-2）：
        // 1-3 及以后的备战页没有独立模板，节点号必须能通过 PaddleOCR
        // 读"备战阶段 X-X"整行解析出来（页面/数字解耦方案）。
        using var characterRecognizer = new OpenCvCharacterCardRecognizer(
                lenientLeadOverCharacterIds:
                [
                    "currency_wars_character_40",
    ...
    public async Task LiveCapturedPreparationProducesVisibleCoreStateWithinRealtimeBudget()
    {
        ...
        Assert.DoesNotContain(state.PendingIcons, item =>
            item.Category == PendingIconCategory.AdvancedEquipment &&
            item.RecognizedFields is { } fields &&
            fields.TryGetValue("ownerCharacterId", out var ownerId) &&
            ownerId == "currency_wars_character_24");
        Assert.Equal(
            EquipmentSlotOccupancy.Empty,
    ...
    [Fact]
    public async Task VisibleStandardEquipmentDoesNotCreateUnknownOrPendingEvidence()
    {
        using var characterRecognizer = new OpenCvCharacterCardRecognizer(
                lenientLeadOverCharacterIds:
                [
        ...
        var state = await analyzer.AnalyzeAsync(
            frame,
            "unknown",
            "fixture:empty-equipment-slot-contract",
            EmptySnapshot(frame.CapturedAt),
            CancellationToken.None);
        ...
        Assert.Collection(
            ...
            slot =>
            {
                ...
                Assert.Equal("currency_wars_equipment_061", slot.EquipmentId);
                // 2026-08-20 用户定性：主装备已精确识别(061)，"候选 [061,100]"不是
        ...
        var ownerPending = state.PendingIcons.Where(item =>
            item.Category == PendingIconCategory.AdvancedEquipment &&
            item.RecognizedFields is { } fields &&
            fields.TryGetValue("ownerCharacterId", out var ownerId) &&
            ownerId == "currency_wars_character_24").ToArray();
        Assert.Empty(ownerPending);
    }
    ...
    public async Task LivePreparationInventoryPreservesDuplicateSimpleItemsAndEmptySlot()
    {
        ...
        Assert.Equal(ObservationStatus.Known, state.InventorySlots.Status);
        // 2026-08-14 识别改进：拆卸工具 153 归入特殊物品槽位，
        // 与 0.2.855 版断言一致（7 槽 + SpecialItemIds 单独校验）。
        Assert.Equal(
            new[]
            {
                (0, EquipmentSlotOccupancy.Equipped,
                ...
                (2, EquipmentSlotOccupancy.Equipped,
                    "currency_wars_equipment_041"),
                (3, EquipmentSlotOccupancy.Equipped,
                    "currency_wars_equipment_040"),
                (4, EquipmentSlotOccupancy.Empty, (string?)null),
                (5, EquipmentSlotOccupancy.Empty, (string?)null),
                (6, EquipmentSlotOccupancy.Empty, (string?)null)
            },
            state.InventorySlots.Value!.Select(item => (
                item.SlotIndex,
                item.Occupancy,
    ...
    [Fact(Skip = "2026-08-20 用户定性：商店界面当前设计=跳过前方角色识别，本测试期望商店页认出前方装备是改方案前遗留过时断言，暂停；待下一版实现商店页识别前方后启用。")]
    public async Task VisibleShopEquipmentWithoutPrivilegeOverlayResolvesStandardIdentity()
    {
        var frame = LoadLiveCapture("preparation-shop-1-4.png");
        using var characterRecognizer = new OpenCvCharacterCardRecognizer(
                lenientLeadOverCharacterIds:
        ...
        var state = await analyzer.AnalyzeAsync(
            frame,
            "reward_shop",
            "fixture:ambiguous-shop-equipment",
            EmptySnapshot(frame.CapturedAt),
            CancellationToken.None);

        var owner = state.Formation.Value!.First(item =>
            item.CharacterId == "currency_wars_character_44");
        var slot = owner.FinalEquipmentSlots[1];
        Assert.Equal(EquipmentSlotOccupancy.Equipped, slot.Occupancy);
        Assert.Equal("currency_wars_equipment_066", slot.EquipmentId);
        Assert.Equal(
            [
        ...
        Assert.False(slot.IsPrivileged);
        Assert.True(slot.CanDriveDecisions);
        ...
        Assert.DoesNotContain(state.PendingIcons, item =>
            ...
            ownerCharacterId == "currency_wars_character_44" &&
            fields.TryGetValue(
                "equipmentSlot",
    ...
    [Fact(Skip = "2026-08-20 用户定性：商店界面当前设计=跳过前方角色识别，本测试期望商店页认出前方装备是改方案前遗留过时断言，暂停；待下一版实现商店页识别前方后启用。")]
    public async Task VerifiedPrivilegeFrameAndVResolvePrivilegeIdentityAndFlag()
    {
        var source = LoadLiveCapture("preparation-shop-1-4.png");
        var region = Phase2RecognitionRegions.CharacterEquipmentSlots(
            Phase2RecognitionRegions.RewardShopCharacterSlots1920[0],
            compactFrontLayout: true)[1];
        var bounds = region.ToPixels(source.Width, source.Height);
        var pixels = (byte[])source.BgraPixels.Clone();
        var thickness = Math.Max(1, Math.Min(bounds.Width, bounds.Height) / 18);
        for (var y = bounds.Y; y < bounds.Bottom; y++)
        {
            for (var x = bounds.X; x < bounds.Right; x++)
            {
                if (x >= bounds.X + thickness && x < bounds.Right - thickness &&
                    y >= bounds.Y + thickness && y < bounds.Bottom - thickness)
                {
                    continue;
                }

                SetPixel(pixels, source.Stride, x, y, 230, 200, 0);
            }
        }
        ...
        for (var segment = 0; segment < 2; segment++)
        {
            ...
            {
                var ratio = step / 16d;
                var x = bounds.X + (int)Math.Round(bounds.Width *
                    (points[segment].X +
                     (points[segment + 1].X - points[segment].X) * ratio));
                var y = bounds.Y + (int)Math.Round(bounds.Height *
                    (points[segment].Y +
                     (points[segment + 1].Y - points[segment].Y) * ratio));
                SetPixel(pixels, source.Stride, x, y, 255, 255, 255);
            }
        }

        var frame = source with { BgraPixels = pixels };
        Assert.True(Phase2OperationalScreenshotAnalyzer
        ...
        var state = await analyzer.AnalyzeAsync(
            frame,
            "reward_shop",
            "fixture:unverified-privilege-marker",
            EmptySnapshot(frame.CapturedAt),
            CancellationToken.None);
        ...
        var slot = owner.FinalEquipmentSlots[1];
        Assert.Equal(EquipmentSlotOccupancy.Equipped, slot.Occupancy);
        Assert.Equal("currency_wars_equipment_105", slot.EquipmentId);
        Assert.Equal(
            [
                "currency_wars_equipment_066",
                "currency_wars_equipment_105"
            ...
            slot.CandidateEquipmentIds);
        Assert.True(slot.IsPrivileged);
        Assert.True(slot.CanDriveDecisions);
        Assert.DoesNotContain(state.PendingIcons, item =>
            item.Category == PendingIconCategory.AdvancedEquipment &&
            item.RecognizedFields is { } fields &&
            fields.TryGetValue("ownerCharacterId", out var ownerId) &&
            ownerId == "currency_wars_character_44");
    }
    ...
    public async Task ExpandedShopAnalyzerKeepsVisibleFormationWithCompactEvidenceRegions()
    {
        ...
        Assert.Equal(Phase2PageFamily.Supply, state.PageFamily);
        Assert.Contains(
    ...
    [Fact]
    public async Task RewardShopIsSupplyInsteadOfPreparation()
    {
        ...
        Assert.Equal(Phase2PageFamily.Supply, result);
    }
    ...
    public async Task RealSettlementReferenceExposesGoldAndTopThreeDamage()
    {
        ...
        // 结算金币奖励识别已废弃（用户 2026-08-06）：恒 Unknown
        Assert.Equal(ObservationStatus.Unknown, state.SettlementGoldReward.Status);
    ...
    [Theory]
    [InlineData("3,545", 3_545L)]
    [InlineData("8,863", 8_863L)]
    public async Task BattleThousandsSeparatorUsesBaseUnits(
        string text,
        long expected)
    {
        var frame = LoadReference("130104.png");
        var target = Phase2RecognitionRegions.BattleDamageValue(0)
            .ToPixels(frame.Width, frame.Height);
        var ocr = new RegionSelectiveOcr(target, text);
        var dataDirectory = Path.Combine(RepositoryRoot, "data", "4.4");
        var gameData = GameDataCatalogLoader.Load(dataDirectory);
        var analyzer = new Phase2OperationalScreenshotAnalyzer(
            new EmptyCharacterRecognizer(),
        ...
        var state = await analyzer.AnalyzeAsync(
            frame,
            "battle_generic",
            $"fixture:battle-thousands:{text}",
            EmptySnapshot(frame.CapturedAt),
            CancellationToken.None);

        Assert.Equal(Phase2PageFamily.Battle, state.PageFamily);
        Assert.Equal(expected, state.BattleScreenDamageCandidate.Value);
    }
    ...
    [Fact]
    public void SettlementDamageAbove100BillionIsNoLongerDropped()
    {
        // C 修复（用户 2026-08-06 深夜确认）：原 1000 亿硬上限会丢弃
        // 3-7 等首领节点大伤害（数千亿量级）。放宽到 1 万亿后应保留。
        // 15000000 万 = 1500 亿（超过原 1000 亿上限，验证放宽生效）。
        var candidates = Phase2OperationalScreenshotAnalyzer
            .ParseSettlementDamageCandidates("15000000万")
            .ToArray();
        var best = candidates.OrderByDescending(item => item.Score).First();

        Assert.Equal(150_000_000_000L, best.Value);
    }

    [Fact]
    public void SettlementDamageWithTrillionUnitIsParsed()
    {
        // 万亿连写支持（review 2026-08-06 建议）：1.5万亿 = 1.5e12。
        var candidates = Phase2OperationalScreenshotAnalyzer
            .ParseSettlementDamageCandidates("1.5万亿")
            .ToArray();
        var best = candidates.OrderByDescending(item => item.Score).First();

        Assert.Equal(1_500_000_000_000L, best.Value);
    }

    [Fact]
    public void SettlementDamageWithExplicitWanUnitKeepsScale()
    {
        // B 修复：结算伤害区域加宽后"万"单位进裁剪，不再走单位推断。
        // 带单位直接解析（score 更高、不 ambiguous）。
        var candidates = Phase2OperationalScreenshotAnalyzer
            .ParseSettlementDamageCandidates("975015.27万")
            .ToArray();
        var best = candidates.OrderByDescending(item => item.Score).First();

        Assert.Equal(9_750_152_700L, best.Value);
        // 带显式单位时 score 应高于无单位推断（3=显式单位 vs 1=推断）。
        Assert.True(best.Score >= 3);
    }
    ...
    public async Task BatchSettlementReportIncludesGoldDamageAndNoRunRecords()
    {
        ...
        try
        {
            ...
            // 结算金币奖励识别已废弃（用户 2026-08-06）：不再产生
            // SettlementGoldReward 识别条目。
            Assert.DoesNotContain(image.Recognitions, item =>
        ...
        int expectedDifficulty)
    {
        ...
        // 2026-08-20 用户定性：软件对备战帧装备已精确识别（非蓝段 matched 多件，
        // conf 0.95），"待补先进装备"只在"软件拿不准某件装备"时才产生，本非每帧
        // 都必须有——此断言是测试期望过时，删除（用户认可，勿恢复）。
    }
        ...
        string fileName)
    {
        ...
        var frame = LoadReference(fileName);
        // 全量对照与短名单用同一 lenient 区分度集合（2026-08-07）：白厄/
        // 飞霄/阿格莱雅/开拓者相似角色 0.01，其他 0.04——否则全量(默认0.04)
        // 与短名单(0.01)对阿格莱雅等判定不一致，测试误报。
        using var exhaustive = new OpenCvCharacterCardRecognizer(
            int.MaxValue,
            [
                "currency_wars_character_40",
                "currency_wars_character_56",
    ...
    public void IconCatalogLoadsExistingAndImportedResourcesWithExplicitVisualAmbiguity()
    {
        ...
        // 2026-08-16：042__live 实机变体模板（黑底蓝图案）加入——
        // simple-equipment 8→9（数据契约随模板更新，非掩盖失败）。
        // 2026-08-20：045__live（生命之花实机变体，黑底蓝图案）加入——
        // simple-equipment 9→10，生命之花实机在后台黑底渲染时需靠它匹配。
        // 2026-08-22：用户用 8 张完整贴边新图标替换 040-047 主模板，并删除
        // 042/045 的 _live 旧变体（识别只认新图）——simple-equipment 10→8。
        Assert.Equal(8, templates.Count(item =>
            item.Category == "simple-equipment"));
        Assert.Equal(71, templates.Count(item =>
            item.Category == "advanced-equipment"));
        // 备注：071 含 080__live（追逐星辰实机变体，前台浅底实机渲染差异）——
        // advanced-equipment 70→71，数据契约随模板更新。
        ...
        // D 步（2026-08-17）：加入 6 个实机截图模板（3 武装箱 + 3 聘书，
        // 命名 {id}__live.png 走 phase2-icon-templates/special-item/）后，
        // special-item 从 29 变为 35。这些模板与官方模板同 id 并存，由
        // 识别器按最小距离择优。
        // 2026-08-20：special_item_010__live（冶金炉实机变体）→ 36。
        Assert.Equal(36, templates.Count(item => item.Category == "special-item"));
        ...
        Assert.Equal(7, templates.Count(item =>
    ...
    public void ReviewedThreeAndFourCostHiringBooksAreEnabledForRecognition()
    {
        ...
        {
            // D 步（2026-08-17）：special_item_020/021 现有一个官方模板 +
            // 一个实机 __live 变体（同 id 并存，识别器按最小距离择优），
            // 故从 Assert.Single 改为"存在且官方模板 ResolvesExactIdentity"。
            var templatesForId = templates.Where(item =>
                item.Id == id &&
                string.Equals(
                    item.Category,
                    "special-item",
                    StringComparison.OrdinalIgnoreCase)).ToArray();
            Assert.NotEmpty(templatesForId);
            var official = Assert.Single(templatesForId.Where(item =>
                !item.FilePath.Contains(
                    "__live",
                    StringComparison.OrdinalIgnoreCase)));
            Assert.True(official.ResolvesExactIdentity);
            Assert.Equal([id], official.CandidateIds);
            Assert.Contains(templates, item =>
                item.Id == id &&
                string.Equals(
                    item.Category,
        ...
        int height)
    {
        ...
        for (var row = 0; row < 8; row++)
        {
            ...
            var damage = Phase2RecognitionRegions.BattleDamageValue(row)
                .ToPixels(width, height);
            // 头像框右缘可与伤害数字区紧贴（用户 2026-08-18 实机确认：本游戏
            // 伤害榜头像框与右侧数字本来就相邻/微重叠约 9px，OCR 仍读全数字）。
            // 断言只需头像框不越出数字区右界，避免旧理想化假设 avatar.Right<=
            // damage.X 在真实贴合布局下误报。
            Assert.True(avatar.Right <= damage.Right, "头像框右缘不应越过伤害数字区右界");
            Assert.True(damage.Right <= width, "伤害数字区不应超出帧宽");
    ...
    [Fact]
    public void SupplyThenOneSixProducesDistinctCorrectMilestones()
    {
        var tracker = new Phase2OperationalStateTracker();
        var supply = PreparationState("1-5", 20) with
        {
            PageFamily = Phase2PageFamily.Supply,
            PageId = "reward_shop"
        };

        tracker.Observe(supply);
        var supplyConfirmed = tracker.Observe(supply);
        tracker.Observe(PreparationState("1-6", 20));
        var preparationConfirmed = tracker.Observe(PreparationState("1-6", 20));

        Assert.True(supplyConfirmed.PageChanged);
        Assert.Contains("进入补给节点 1-5", supplyConfirmed.Message);
        Assert.DoesNotContain("备战", supplyConfirmed.Message);
        Assert.True(preparationConfirmed.PageChanged);
        Assert.Contains("进入备战节点 1-6", preparationConfirmed.Message);
    }

    [Fact]
    public void OneSixPreparationCorrectsMislabeledBattleToOneFourAfterOneThree()
    {
        var tracker = new Phase2OperationalStateTracker();
        tracker.Observe(BattleState("1-3", [100_000], 1, 90));
        tracker.Observe(BattleState("1-3", [200_000], 1, 80));
        tracker.Observe(PreparationState("1-4", 20));
        var firstFinal = tracker.Observe(PreparationState("1-4", 20));
        Assert.Equal("1-3", firstFinal.FinalizedBattle?.NodeId);

        tracker.Observe(BattleState("1-6", [300_000], 1, 70));
        tracker.Observe(BattleState("1-6", [400_000], 1, 60));
        var firstExit = tracker.Observe(PreparationState("1-6", 20));
        var secondExit = tracker.Observe(PreparationState("1-6", 20));
        var corrected = firstExit.FinalizedBattle is not null
            ? firstExit
            : secondExit;

        Assert.Equal("1-4", corrected.FinalizedBattle?.NodeId);
    }
    ...
    [Fact]
    public void UnknownFrameBetweenPreparationFramesDoesNotResetPageConfirmation()
    {
        // 问题1回归防护（2026-08-05 实机 09:40:05→09:41:41 备战延迟 18 秒）：
        // 重新进入对局后，备战页第一帧已识别（fast 判 preparation），
        // 紧接着一帧 __classifier-miss__（Unknown）会把页面确认计数清零，
        // 导致备战确认从头累计，界面"战斗→备战"状态更新被推迟十几秒。
        // 修复：Unknown/Transition 帧只清字段指纹确认，保留页面确认进度
        // （页面确认只在同页面连续帧递增，不同页面帧会重置，保留安全）。
        var tracker = new Phase2OperationalStateTracker();
        var preparation = PreparationState("3-2", 58);

        // 第一帧备战：页面确认计数 = 1，字段确认 = 1
        tracker.Observe(preparation);
        // 一帧 Unknown（分类器 miss 的过渡帧）
        tracker.Observe(new Phase2OperationalState
        {
            PageFamily = Phase2PageFamily.Unknown,
            PageId = "__classifier-miss__"
        });
        // 第二帧备战：页面确认应直接从 1→2 触发 PageChanged
        var confirmed = tracker.Observe(preparation);

        Assert.True(confirmed.PageChanged);
        Assert.Contains("进入备战节点 3-2", confirmed.Message);
    }

    [Fact]
    public void UnknownFrameDoesNotConfirmAnythingByItself()
    {
        // Unknown/Transition 帧本身绝不能触发确认（字段不可信）。
        var tracker = new Phase2OperationalStateTracker();
        var unknown = new Phase2OperationalState
        {
            PageFamily = Phase2PageFamily.Unknown,
            PageId = "__classifier-miss__"
        };

        var first = tracker.Observe(unknown);
        var second = tracker.Observe(unknown);

        Assert.False(first.PersistentStateConfirmed);
        Assert.False(second.PersistentStateConfirmed);
        Assert.False(first.PageChanged);
        Assert.False(second.PageChanged);
    }

    [Fact]
    public void TransitionFrameAlsoPreservesPageConfirmationProgress()
    {
        // Transition 帧与 Unknown 帧走同一早退分支（Observe 开头），
        // 同样只清字段指纹、保留页面确认进度。
        var tracker = new Phase2OperationalStateTracker();
        var preparation = PreparationState("3-4", 30);
        ...
        Assert.True(confirmed.PageChanged);
        Assert.Contains("进入备战节点 3-4", confirmed.Message);
    }

    [Fact]
    public void PageSwitchAfterUnknownFramesRestartsConfirmationCount()
    {
        // 回归防护：Unknown 帧保留页面确认进度后，若页面确实切换
        // （备战→战斗），新页面第一帧计数必须重置为 1，不能按旧页面
        // 残留计数提前确认——否则会误报"进入备战节点 X"或"战斗开始"。
        var tracker = new Phase2OperationalStateTracker();
        var preparation = PreparationState("3-2", 58);
        ...
        // 页面切换：进入战斗。第一帧战斗不应因残留计数立即确认 PageChanged。
        var battle = BattleState("3-2", [1_000_000], 1, 95);
        var firstBattle = tracker.Observe(battle);

        Assert.False(firstBattle.PageChanged);
        // 第二帧战斗才确认
        var secondBattle = tracker.Observe(battle);
        Assert.True(secondBattle.PageChanged);
        Assert.Contains("战斗开始", secondBattle.Message);
    }
    ...
    [Fact]
    public void ShopOverlayDoesNotTriggerPrematureBattleFinalization()
    {
        // 问题5回归防护：2-2 战斗未开始，商店页（reward_shop）被归为
        // Preparation，旧逻辑会把残留战斗帧误封存为"2-2 战斗已结束"。
        // 商店/策略页是弹窗（可能遮住节点锚点），不应触发非战斗退出封存。
        var tracker = new Phase2OperationalStateTracker();
        // 2-1 战斗结束，残留战斗帧在缓冲中
        tracker.Observe(BattleState("2-1", [100_000], 1, 90));
        tracker.Observe(BattleState("2-1", [200_000], 1, 80));
        // 进入 2-2 备战页（正常应封存 2-1）
        tracker.Observe(PreparationState("2-2", 50));
        var prepConfirmed = tracker.Observe(PreparationState("2-2", 50));
        // 打开商店（reward_shop），确认帧——不应触发新的封存
        var shopFrame = PreparationState("2-2", 50) with
        {
            PageId = "reward_shop"
        };
        tracker.Observe(shopFrame);
        var shopConfirmed = tracker.Observe(shopFrame);

        Assert.NotNull(prepConfirmed.FinalizedBattle);
        Assert.Equal("2-1", prepConfirmed.FinalizedBattle!.NodeId);
        // 商店确认帧不应产生新的封存（2-2 战斗未开始）
        Assert.Null(shopConfirmed.FinalizedBattle);
    }

    [Fact]
    public void StrategyOverlayDoesNotTriggerPrematureBattleFinalization()
    {
        // 问题5同类：投资策略页（investment_strategy）同样不应触发封存。
        var tracker = new Phase2OperationalStateTracker();
        tracker.Observe(BattleState("1-9", [100_000], 1, 90));
        tracker.Observe(BattleState("1-9", [200_000], 1, 80));
        tracker.Observe(PreparationState("1-8", 30));
        var prepConfirmed = tracker.Observe(PreparationState("1-8", 30));
        var strategyFrame = PreparationState("1-8", 30) with
        {
            PageId = "investment_strategy"
        };
        tracker.Observe(strategyFrame);
        var strategyConfirmed = tracker.Observe(strategyFrame);

        Assert.NotNull(prepConfirmed.FinalizedBattle);
        Assert.Null(strategyConfirmed.FinalizedBattle);
    }
    ...
    public void SettlementFinalizationKeepsBothCandidatesAndSelectsMaximum()
    {
        ...
        Assert.Null(finalized.GoldReward); // 结算金币奖励已废弃
    ...
    {
        var tracker = new Phase2OperationalStateTracker();
        // 1-1/1-2 是奖励关（用户 2026-08-11 拍板必然过关 IsComplete），
        // 该用例验证"仅 context 战斗帧"的普通节点封存语义，改用 2-1。
        var battle = BattleContextState("2-1", 0, 80);
        var settlement = new Phase2OperationalState
        {
            PageFamily = Phase2PageFamily.BattleSettlement,
            NodeId = Observation<string>.Known("2-1", 0.9)
        };
        tracker.Observe(battle);
        tracker.Observe(battle);
    ...
    public async Task UnknownPagePreservesReadableIndependentRegions()
    {
        ...
        // 2026-08-11 用户拍板：unknown 页不做降级 OCR（原
        // AnalyzeUnknownRegionsAsync 全区域识别阻塞管线）。
        Assert.Empty(state.PartialFields);
    }
            ...
            GameDataCatalog gameData)
    {
        ...
        // 多变体加载（2026-08-07）：同角色可有多张卡面模板（银狼 3/4/5 费、
        // 开拓者记忆/欢愉、用户实机变体）——全部加载，匹配取最高分。
        // 旧逻辑 .Single() 遇多变体抛异常（全量回归 69 失败根因）。
