...
public static class CaptureFramePreprocessor
{
    ...
    /// <summary>
    /// 裁剪并缩放到目标高度（宽度等比）。用于超大字形（如新版商店等级
    /// 161px 大数字）：放大 4x 会超出数字模板尺度，缩放到 targetHeight
    /// 后进入模板舒适区（2026-08-15 商店等级双布局修复）。
    /// </summary>
    public static CaptureFrame CreateScaledCrop(
        CaptureFrame frame,
        PixelRect region,
        int targetHeight)
    {
        ...
        var targetWidth = Math.Max(
            1,
            (int)Math.Round(
                bounded.Width * ((double)targetHeight / bounded.Height)));
        using var resized = new Mat();
        Cv2.Resize(
            crop,
            resized,
