...
public sealed class Phase2LiveCollectionService(
    ...
    IPhase2RecognitionFeed? recognitionFeed = null) :
    IPhase2LiveCollectionService
{
    ...
    /// <summary>
    /// 普通帧观察写盘的串行队列（持久化通道，三语义通道之一）：
    /// PNG/裁剪/JSON 写盘从收集主循环解耦——主循环不再被磁盘 IO 阻塞，
    /// 队列有界且 DropOldest（普通帧画面过期后写最新即可，关键帧
    /// （封存/终局）仍同步 await 写盘，绝不入队丢弃）。
    /// </summary>
    private readonly Channel<ObservationSaveRequest> _saveQueue =
        Channel.CreateBounded<ObservationSaveRequest>(
    ...
    // 不代表完成（可能是手动退出），不能据此封存。
    // 根因（2026-08-06 排查）：原探测在 tracker.Observe() 之前执行，
    // 评级页帧到达时 LastFinalizedNode 还是 3-6 → 探测永不触发；
    // 之后 home 帧又识别不到评级页特征 → 整局永不自动封存。
        ...
        CancellationToken cancellationToken)
    {
        ...
        var evidenceFrameCache = new Phase2FinalEvidenceFrameCache(runId);
        FinalNodeBattleState? pendingFinalPersistence = null;
        ...
        _pendingRatingPageConfirmation = false;
        ...
        if (recognitionWarmUp is not null || phase2Ocr is not null)
        {
            ...
            {
                if (recognitionFeed is Phase2RecognitionFeed warmUpFeed)
                {
                    warmUpFeed.Detach();
                }
        ...
        try
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (pipelineUpdate.IsHeartbeat)
                    {
                            ...
                            pipelineUpdate.Frame is not null)
                        {
                            ...
                            pendingFinalPersistence ??=
                                heartbeatTracking.FinalizedBattle;
                            if (heartbeatTracking.FinalizedBattle is null &&
                                pendingFinalPersistence is not null)
                            {
                                heartbeatTracking = heartbeatTracking with
                                {
                                    FinalizedBattle = pendingFinalPersistence,
                                    Message =
                                        "Retrying durable persistence for the finalized node."
                                };
                            }
                            ...
                            {
                                await evidenceFrameCache.PersistFinalEvidenceAsync(
                                ...
                                evidenceFrameCache.RetainOnly(
                                    tracker.ActiveBattleEvidenceSourceIds);
                            }
                            else
                            {
                                evidenceFrameCache.RetainOnly(
                    ...
                    _ = evidenceFrameCache.Register(screenshotName, frame);
                var completionKind = Phase2RunCompletionDetector.Classify(
                        analysis,
                        tracker.ActiveBattleNode ?? tracker.LastFinalizedNode);
                // 3-7 整局评级页探测（用户 2026-08-06 深夜最终方案）：
                // 探测到 3-7 战斗结束（finalize，由 tracker.Observe 置位
                // _pendingRatingPageConfirmation）后，只有语义分类命中评级页
                // （challenge_success + "下一步"特征）才算封存整局；
                // home 页面不代表完成（可能是手动退出），不能据此封存。
                // 根因修复：原逻辑在 Observe 之前查 LastFinalizedNode==3-7，
                // 评级页帧到达时还是 3-6 → 探测永不触发 → 整局永不封存。
                if (completionKind != Phase2RunCompletionPageKind.FinalSuccess &&
                    _pendingRatingPageConfirmation &&
                    analysis.Snapshot.PageId.Status == ObservationStatus.Known &&
                    analysis.Snapshot.PageId.Value is
                        "challenge_success" or "currency_wars_home" or "battle_generic" &&
                    pipelineUpdate.Frame is not null &&
                    phase2Ocr is not null)
                {
                    var semantic = await Phase2SettlementSemanticClassifier
                        .TryClassifyAsync(
                            pipelineUpdate.Frame,
                            phase2Ocr.Text,
                            cancellationToken)
                        .ConfigureAwait(false);
                    var isRatingPage = semantic?.PageId == "challenge_success" &&
                                       semantic.Evidence.Any(item =>
                                           item.Contains(
                                               "下一步",
                    ...
                    {
                        _pendingRatingPageConfirmation = false;
                        completionKind = Phase2RunCompletionPageKind.FinalSuccess;
                    }
                }
                        ...
                        Phase2RunCompletionPageKind.FinalSuccess)
                {
                    ...
                    // 失败，但不能把它一律改写成“生命耗尽”。
                    var finalBattle = pendingFinalPersistence ??
                                      tracker.CompletePendingBattle(
                                          analysis.Snapshot.Health);
                    pendingFinalPersistence ??= finalBattle;
                ...
                if (analysis.OperationalState is not null)
                {
                    tracking = tracker.Observe(
                        ...
                        analysis.Snapshot.Health);
                    pendingFinalPersistence ??= tracking.FinalizedBattle;
                    if (tracking.FinalizedBattle is null &&
                        pendingFinalPersistence is not null)
                    {
                    ...
                    analysis = analysis with
                    {
                        ...
                        // 商店等级桥接：tracker 合并了 state.StoreLevel，
                        // 但 snapshot.StoreLevel 无源头填充（仅即时识别分析器
                        // 曾直接写入）。把 tracker 合并结果桥接到 snapshot，
                        // 否则实时报告/节点历史商店 Lv 恒"未记录"
                        //（P1-8：识别已接入 Phase2OperationalScreenshotAnalyzer，
                        // 这里补 snapshot 传播）。
                        // Known 或 Stale 都桥接（tracker 的 Carry 在 OCR 偶发
                        // 失败帧输出带值 Stale；只认 Known 会让该帧退化为
                        // Unknown——review 2026-08-06 抓到）。
                        Snapshot = tracking.Current.StoreLevel.Status is
                                   ObservationStatus.Known or
                                   ObservationStatus.Stale
                            ? analysis.Snapshot with
                            {
                                StoreLevel = tracking.Current.StoreLevel
                            }
                            : analysis.Snapshot
                    };
                    if (tracking.FinalizedBattle is not null)
                    {
                        await evidenceFrameCache.PersistFinalEvidenceAsync(
                                ...
                                cancellationToken)
                            .ConfigureAwait(false);
                        // 3-7 刚 finalize：允许后续评级页探测触发整局封存。
                        // 必须在 tracker.Observe 之后判定（用户 2026-08-06）：
                        // 原探测在 Observe 之前跑，评级页帧到达时 LastFinalizedNode
                        // 还是 3-6，导致 3-7 打完永不自动封存（实测手动结算）。
                        if (string.Equals(
                                tracking.FinalizedBattle.NodeId,
                                "3-7",
                                StringComparison.OrdinalIgnoreCase))
                        {
                            _pendingRatingPageConfirmation = true;
                        }
                    ...
                    {
                        // 新 run 边界：清除 3-7 评级页待确认标志，防止跨 run 残留
                        //（review 2026-08-06 warn 建议）。
                        _pendingRatingPageConfirmation = false;
                        var previousRunId = runId;
                        var boundaryAt = DateTimeOffset.UtcNow;
                        var previousRunWasCompleted = currentRunCompleted;
                        if (tracking.FinalizedBattle is { } boundaryFinal)
                        {
                            // This final belongs to the old run. Its referenced
                            // frames and node JSON must be durable before the
                            // run id and cache are reset.
                        ...
                        evidenceFrameCache =
                            new Phase2FinalEvidenceFrameCache(runId);
                        currentRunCompleted = false;
                        postCompletionBoundary.Reset();
                        analysis = WithRunId(analysis, runId) with
                        {
                ...
                if (shouldSaveRecognized || shouldSaveDegraded)
                {
                    ...
                    {
                        // 普通帧异步写盘：收集主循环不被磁盘 IO 阻塞。
                        EnqueueObservationSave(new ObservationSaveRequest(
                            frame,
                            screenshotName,
                            analysis,
                            null));
            ...
            {
                var retainedSources = pendingFinalPersistence is null
                    ? tracker.ActiveBattleEvidenceSourceIds
                    : Phase2FinalEvidenceSourceCollector.Collect(
                        pendingFinalPersistence);
                evidenceFrameCache.RetainOnly(retainedSources);
                consecutiveFailures++;
                        ...
                        failureNow))
                {
                    ...
                    Publish(
                        ...
                        isError: true);
                    if (recognitionFeed is Phase2RecognitionFeed pausedFeed)
                    {
                        pausedFeed.Detach();
                    }
        ...
        Publish(
            ...
            "只读局势收集已停止。",
            isMilestone: true);
        if (recognitionFeed is Phase2RecognitionFeed feedHost)
        {
            feedHost.Detach();
        }
    }

    internal sealed record ObservationSaveRequest(
        CaptureFrame Frame,
    ...
    private async Task PumpObservationSavesAsync(
        CancellationToken cancellationToken)
    {
        await foreach (var request in _saveQueue.Reader
