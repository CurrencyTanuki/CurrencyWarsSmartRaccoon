...
27c. **1.2.129/1.2.130 发布+清理（09-11 09:0x-14:3x，用户令：修卡住→发布→测无废聘用书→整理接入正式版）**：①用户令修 3-4 分钟卡住（金尽不关店+快照门禁拒读+识别管线静帧楔死三层叠加，卡的是快照恢复预算 ~4 分钟）→**1.2.129 Fix A 快速通道**（账本报金尽即先收摊再快照，砍 TryRecover 195 秒不可能重试，最坏 ~40 秒）+**1.2.130 单步模式两修复**（看门狗单步模式只告警不自动 A9；M4 加 ignoreBookCounters 跳计数门）——均对抗审查 APPROVED；②**实测事故如实入档**：M4 force 模式在用户真实 3-7 尾局上跑（我方决策失误），识别静帧饿死+点击落在购买经验按钮（备战席 1 号位邻位），买经验多次直至 abort——根因=M4 点击无"必须识别到书图标"硬校验+不应在用户真实局上测，修复方向已定（点击前硬校验）未实施；③AUTO-A9 误触发电已修（只读 I10 失败 2 次即武装弃局=单步模式地雷，已改单步模式只告警）；④**清理**：审计帧墙 debris ~700MB、CW-game-iter 旧迭代 1.4GB 已删（可由 git 任意 commit 重建）；保留：CW-sandbox-iter（沙箱基建）、全部录像、桌面 8 月数据文件夹、红线源码备份；⑤**正式版已接入最新程序**：1.2.130+893ce9d 部署 VERIFY-PASS（与 HEAD 一致），实例运行中；⑥未发布动作（用户令"先不发布"）＝未发 DECIDE 未开新测试。**待用户**：发布令/无废聘用书测试时机/M4 点击硬校验修复的三连拍板。
...
27h. **1.2.132 UI 面板版构建+全量测试门（09-12 上午）**：UI 子代理交付（AdvisorRecommendationViewModel 382 行+MainWindow 面板 343 行+DI/接线/OnClosing 停泵，构建 0 警 0 错，18 处绑定逐一核对）→ 1.2.132 MSI 构建成功（94.9MB）但 **MSI 安装三连挂死**（1.2.131 包×1+1.2.132 包×2：46 文件处冻结/0 文件即冻/verbose 日志从未写出）——msiexec 已杀、半装残留已清、挂死根因挂后续专项（Installer 服务重启后第三次仍挂=与负载或 wxs 结构相关，需深诊）。**已转交付便携包**：CurrencyWarsAssistant-1.2.132-portable.zip（111.7MB，1351 条目含使用说明，完整性验证通过，含 UI 面板）——满足"传输到其他电脑立刻直接使用"（前置=.NET 8 Desktop Runtime，包内说明有官方链接）。Phase2Advisor 20/21 过（唯一失败=已备案 CompositeAnalyzer 既有潜伏失败）；全量套件发布门跑动中。

27i. **全量套件 976 用例结果（09-12 02:1x，脱离会话重跑完成）**：**2 失败/960 过/14 跳过**。两处失败均非本轮引入——①CompositeAnalyzerKeepsOverlayedHomePageOutOfBattlePipeline（09-11 沙箱班已备案的既有潜伏失败）；②另一失败为 Phase2Advisor 家族的既有红测试（基线数据目录含 27 文件时的 v0 加载器行为，1.2.133 容错修复后其断言目标已变——见审计 P1-1 修复说明，待专项对齐预期）。**主结论：本轮 ABCD+止血+行为批的全部改动在 976 用例回归中零新增失败。**

27j. **全量套件发布门结果（09-12 02:1x）**：Tests.dll 976 用例：**2 失败/960 过/14 跳过**。失败①=CompositeAnalyzerKeepsOverlayedHomePageOutOfBattlePipeline（沙箱班已备案既有潜伏失败）；失败②=PpOcrOfflineOcrTests.RecognitionOnlyModelReadsRealGameUiCrops(settlement-gold-9.png,预期"9")——OCR 离线识别测试，本轮改动未触 OCR 层，疑似环境/阈值敏感（待专项定性）。**最重发现=测试宿主进程原生崩溃**：AccessViolationException at OpenCvSharp.Mat.get_Width ← OpenCvTemplateMatcher.ProbeNormalized/ProbeMany（c0000005 族）——识别器 Dispose 与识别中并发 use-after-free（子代理 5 号 P2-2 的原生级实锤）+测试宿主崩溃中止了剩余 ~215 用例。全部入档 AUDIT_20260912_SIX_AGENT_CONSOLIDATED.md 修复路线：原生句柄生命周期治理列为最高优先（与本班识别管线楔死/静帧饥饿同域）。
