...
- **E2E 首跑后下一步**（用户已定方向"素材是最大卡点+别当大东西开发"）：素材已齐（祈愿 3 档帧+聘用书选人页 video2-focus 21 帧+备战/商店/策略 PageReplay 全有）——下一步=写**祈愿应答专项迷你脚本**（备战帧起手→M3 发令→弹框应答→期望=点选试炼卡+确认，四档循环）→E2E→再拼 1-3 备战段全脚本。**素材问题 vs 软件问题判纪**（用户令）：识别失败先查帧质（字幕遮挡/动画未加载/分辨率）再疑软件；逻辑判断错误才是软件问题。
