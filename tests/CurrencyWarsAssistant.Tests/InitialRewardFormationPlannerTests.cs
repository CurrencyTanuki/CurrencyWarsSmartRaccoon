...
public sealed class InitialRewardFormationPlannerTests
{
    ...
    [Fact]
    public void PrioritizesFateGrailAndDoesNotLimitCount_AcrossInitialAndSupplementalFormation()
    {
        // 2026-08-26 用户裁决：命杯成员要"越多越好、优先上场"，不再限制最多 1 名
        // （祈愿试炼弹框已全局适配）。初始布阵应优先选命杯（凛、闪），填满 3 人口。
        var accepted = new HashSet<string>(
            ["远坂凛", "吉尔伽美什", "万敌", "风堇"],
            StringComparer.OrdinalIgnoreCase);
        ...
        // 两名命杯成员（凛、闪）都应上场（越多越好，不再限 1 名）。
        Assert.Equal(2, initial.Placements.Count(item =>
        ...
        // 场上已有凛（命杯），补充时应优先补命杯成员吉尔伽美什（越多越好）。
