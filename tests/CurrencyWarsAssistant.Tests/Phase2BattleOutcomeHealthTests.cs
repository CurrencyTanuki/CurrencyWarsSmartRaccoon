...
public sealed class Phase2BattleOutcomeHealthTests
{
        ...
        int rawTerminalAction)
    {
        ...
        ObserveTwice(tracker, Preparation("2-1"), 84);
        ObserveTwice(tracker, Battle("2-1", rawTerminalAction), null);

        tracker.Observe(Preparation("2-2"), Health(80));
        var update = tracker.Observe(Preparation("2-2"), Health(80));
    ...
    public void UnchangedHealthAlone_DoesNotClaimPerfectClear()
    {
        var tracker = new Phase2OperationalStateTracker();
        ObserveTwice(tracker, Preparation("2-1"), 84);
        ObserveTwice(tracker, BattleContext("2-1", 3), null);

        tracker.Observe(Preparation("2-2"), Health(84));
        var update = tracker.Observe(Preparation("2-2"), Health(84));
    ...
    public void MissingHealth_WithMoreThanTenActions_MarksPerfect()
    {
        ...
        ObserveTwice(tracker, Battle("2-1", 55), null);
    ...
    public void MissingHealth_WithTenActions_RemainsUnknown()
    {
        var tracker = new Phase2OperationalStateTracker();
        ObserveTwice(tracker, Preparation("2-1"), null);
        ObserveTwice(tracker, Battle("2-1", 10), null);
    ...
    public void RewardBattle_AcceptsSemanticallyAnchoredChallengeSuccessGold()
    {
        ...
        // 结算金币奖励已废弃（用户 2026-08-06）：恒 null
        Assert.Null(Assert.IsType<FinalNodeBattleState>(update.FinalizedBattle).GoldReward);
    }
    ...
    {
        var tracker = new Phase2OperationalStateTracker();
        ObserveTwice(tracker, Preparation("1-3"), 82);
        ObserveTwice(tracker, Battle("1-3", 0), null);
        var settlement = new Phase2OperationalState
        {
            PageFamily = Phase2PageFamily.BattleSettlement,
            PageId = "challenge_success",
            NodeId = Observation<string>.Known("1-3", 0.95)
        };
        tracker.Observe(settlement, Health(80));
        var firstExit = tracker.Observe(Preparation("1-4"), Health(40));
        var secondExit = tracker.Observe(Preparation("1-4"), Health(40));
        var update = firstExit.FinalizedBattle is not null ? firstExit : secondExit;
    ...
    {
        var tracker = new Phase2OperationalStateTracker();
        ObserveTwice(tracker, Preparation("1-3"), 84);
        ObserveTwice(tracker, Battle("1-3", 0), null);
        tracker.Observe(Preparation("1-4"), Health(80));
        tracker.Observe(Preparation("1-4"), Health(80));
        Assert.Equal("1-3", tracker.LastFinalizedNode);

        // Simulate the delayed node label seen in the user's failure: all
        // pending evidence says 1-9 even though this is the 1-4 battle.
        ObserveTwice(tracker, Battle("1-5", 0), null);
        tracker.Observe(SettlementWithGold("1-5", 74, "single-yellow"), Health(76));
        var update = tracker.Observe(Preparation("1-5"), Health(76));

        var final = Assert.IsType<FinalNodeBattleState>(update.FinalizedBattle);
        Assert.Equal("1-4", final.NodeId);
        Assert.Equal(80, final.PreBattleHealth);
        Assert.Equal(76, final.PostBattleHealth);
        Assert.Equal(-4, final.HealthDelta);
        Assert.Null(final.GoldReward); // 结算金币奖励已废弃
    }
    ...
    [Fact]
    public void SettledBattleWithCompleteFramesMarksNodeComplete()
    {
        // 回归测试（review 二次 blocking）：结算金币奖励废弃后，
        // 战斗帧完整 + 结算页的节点必须标记 IsComplete=true
        //（否则正常通关节点恒 false，节点数据永不完整）。
        var tracker = new Phase2OperationalStateTracker();
        ObserveTwice(tracker, Preparation("1-4"), 80);
        ObserveTwice(tracker, Battle("1-4", 0), null);
        tracker.Observe(SettlementWithGold("1-4", 8, "overview-frame"));
        ...
        Assert.True(final.CanDriveDecisions);
        Assert.Null(final.GoldReward); // 结算金币奖励已废弃
    }
