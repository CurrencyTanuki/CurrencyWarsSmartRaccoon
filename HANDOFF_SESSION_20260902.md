...
## 〇-14、09-10 深夜：二轮 P2 处置+原审查员确认+1.2.123 发布上线（全部根因修复收口）

- **二轮 P2 处置**（ecc36cf）：①P2-1 Kill 段隔离——无 moov 段移入 GrailRecordingTemp/damaged/（不再冒充正常封箱段进 Recordings）+Kill 后补 WaitForExit(3000)；②P2-2 R3 裁决金必须来自 15s 内新鲜读数（PeekGold().CapturedAt 判定），塌缩 0/滞后读数防御跳过判死。
- **原审查员确认**：两处修复逐项判定通过——damaged 隔离自愈链完好；ledgerFresh 门与塌缩机制"严格对偶"（≤15s 必真实读数可判死，>15s 必塌缩 0 必跳过），无循环无新误弃窗口；代价仅"晚弃"（可逆有界，30 轮上限收束），与 1.2.116 立项口径一致。两条 P3 备案：15s 常量与 StaleAfter 无关联（下批共享常量）；13-15s 真实低金读数在持续高滞后下 R3 推迟（时间成本非正确性）。
- **发布**：**1.2.123+ecc36cf** DEPLOY-OK+VERIFY-PASS（09-10 00:52），实例运行中+autodecide 待命；三通道证据核验（GrailRecordingTemp 8 段在录）。**下一轮挂机即验证**：①录像逐局落盘（每局边界封箱到 Recordings，damaged/ 隔离坏段）；②商店识别与画面一致率 100%；③祈愿全应答≤3s；④徽章 700ms 档成功率；⑤无金币账本跳变。
- **挂账（下一批）**：决策层关键 emit 落 jsonl（R3 判死/清场/买经验，事件分类设计）；15s 常量共享；金 13 卖卡存疑；84s 空窗前扩；M5 循环徽章重试（增强）；沙箱分析页对齐（检查点见〇-13a，待占屏窗口）；[X,X,miss,Y] 门级用例。
- **失误记录**：素材清理时 keep/ 建目录失败而删除已执行——g_0770 目击帧与 2.3GB 录像无副本删除（发现已完整入档，仅确认性帧损失）；教训=删除命令必须先验证保留步骤成功。
