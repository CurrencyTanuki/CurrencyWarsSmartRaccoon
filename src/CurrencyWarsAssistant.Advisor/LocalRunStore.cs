...
public sealed class LocalRunStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // 写入必须用无 BOM 的 UTF-8：sha256 用 Encoding.UTF8.GetBytes(json)
            // 计算（无 BOM），若文件带 BOM（Encoding.UTF8 默认 emit BOM）则
            // .sha256 与实际文件字节永远不匹配，误导为"写入中断/归档损坏"
            // （实测 run-20260805-005633 即此现象，文件本身完好）。
            await File.WriteAllTextAsync(
                    temporaryPath,
                    json,
