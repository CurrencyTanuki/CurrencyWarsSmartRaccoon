...
public sealed class CharacterCardRecognizerTests
{
    ...
    public void RecognizesUserFailureReplayAndSeparatesBaiEFromFeixiao()
    {
        ...
        Assert.All(
            results.Skip(4),
            item => Assert.Equal(CharacterCardSlotState.Empty, item.State));
        // 多变体模板（开拓者记忆/欢愉 user 变体）加入后，白厄槽位的
        // runner-up 从"飞霄"变为"开拓者"（银灰发系相近）——测试意图是
        // 验证"白厄不与自身/混淆"，runner-up 具体是谁随模板集变化。
        // 断言改为：runner-up 不是白厄自身（区分有效）且置信度充足。
        Assert.NotEqual("白厄", results[3].RunnerUpDisplayName);
        Assert.True(results[3].Confidence >= 0.58);
        // 多变体后白厄/开拓者相似，runner-up 分差被压到 0.013——阈值对齐
        // lenient 区分度门（0.01）：能区分即可（用户 2026-08-07 授权相似
        // 角色单独放宽）。
        Assert.True(
            results[3].Confidence - results[3].RunnerUpConfidence >= 0.01);
    }
    ...
    public void RecognizesLiveBenchReplays()
    {
        ...
        foreach (var test in cases)
        {
            ...
            {
                // 多变体后 runner-up 可能从"白厄"变为"开拓者"（用户开拓者
                // 模板与飞霄/白厄相似）——测试意图是验证"飞霄不与自身混淆"，
                // runner-up 具体是谁随模板集变化（2026-08-07）。
                Assert.NotEqual("飞霄", results[2].RunnerUpDisplayName);
                // 多变体后飞霄/开拓者相似，runner-up 分差被压到 0.047——阈值
                // 对齐 lenient 区分度门（0.01）：能区分即可（用户 2026-08-07）。
                Assert.True(
                    results[2].Confidence -
                    results[2].RunnerUpConfidence >= 0.01);
            }
        ...
        Assert.All(
            ...
            item =>
            {
                Assert.Equal(
                    CharacterCardSlotState.SpecialOccupied,
                    item.State);
                // 2026-08-08 行为变更：SpecialOccupied 输出模板 id
                //（报告才能显示特殊单位名字，佩佩/特权武装箱同理）
                Assert.Equal(
