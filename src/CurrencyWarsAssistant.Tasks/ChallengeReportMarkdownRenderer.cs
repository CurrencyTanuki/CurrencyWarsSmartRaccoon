    ...
    ChallengeReportModelBuilder modelBuilder)
{
    public string Render(ChallengeReportDocument document)
    {
        ...
        builder.AppendLine("## 节点运营与结果")
            .AppendLine()
            .AppendLine("| 节点 | 最终伤害 | 伤害来源 | 剩余行动值 | 金币 | 金币变化 | 连胜数 | 花费变化 | 血量变化 | 通关 | 理论出伤极限 | 质量 |")
            .AppendLine("|---|---:|---|---:|---:|---:|---:|---:|---:|---|---:|---|");
        foreach (var node in document.Nodes)
        {
            ...
            builder.Append("| ").Append(Escape(node.NodeId))
                ...
                .Append(" | ").Append(battle?.HealthDepleted == true
                    ? "下降（未知）" : healthDelta.Uncertainty is null
                        ? FormatSigned(healthDelta.Value)
                        : "未知（异常增量已拒绝）")
                .Append(" | ").Append(ClearName(battle?.ClearStatus))
                .Append(" | ").Append(hasActionLimit
                    ? FormatDamage(battle?.TheoreticalDamageLimit) : "不适用")
                .Append(" | ").Append(hasActionLimit ? QualityName(battle) : "不适用")
                .AppendLine(" |");
        }
    ...
    private void AppendOperational(StringBuilder builder, ChallengeReportNode node)
    {
        ...
        builder.AppendLine($"- 金币：{FormatInteger(node.Gold)}；金币变化：{FormatSigned(node.GoldDelta)}")
            .AppendLine($"- 连胜数：{FormatInteger(node.CumulativeSpend)}；花费变化：{FormatSigned(node.SpendDelta)}")
    ...
    private void AppendDamage(StringBuilder builder, ChallengeReportNode node)
    {
        ...
        var healthDelta = BattleHealthDeltaPolicy.Evaluate(
            ...
            battle.PostBattleHealth,
            battle.HealthDelta);
        var theoreticalDamage = hasActionLimit
            ? FormatDamage(battle.TheoreticalDamageLimit) : "不适用";
        var theoreticalRule = hasActionLimit
            ? TextOrUnknown(battle.TheoreticalDamageRule)
            : "不适用（该节点没有行动值上限）";
        var maximumAction = hasActionLimit
            ? $"基础 {FormatInteger(battle.BaseMaximumActionValue)}；确认增加 {FormatInteger(battle.ConfirmedActionIncrease)}；有效 {FormatInteger(battle.EffectiveMaximumActionValue)}；理论值质量 {battle.TheoreticalDamageQuality}"
            : "基础 不适用；确认增加 不适用；有效 不适用；理论值质量 不适用";
        builder.AppendLine($"- 最终伤害：{FormatDamage(node.FinalDamage)}；战斗候选：{FormatDamage(battle.BattleScreenDamageCandidate)}；结算候选：{FormatDamage(battle.SettlementScreenDamageCandidate)}")
            .AppendLine($"- 剩余行动：{FormatInteger(battle.RemainingActionValue?.TotalActionValue)}；理论出伤极限：{theoreticalDamage}；理论规则：{theoreticalRule}")
            .AppendLine($"- 最大行动：{maximumAction}")
            .AppendLine(battle.HealthDepleted
                ? $"- 血量：{FormatInteger(battle.PreBattleHealth)} → 已耗尽（具体变化未知）"
                : healthDelta.Uncertainty is null
                    ? $"- 血量：{FormatInteger(battle.PreBattleHealth)} → {FormatInteger(battle.PostBattleHealth)}（{FormatSigned(healthDelta.Value)}）"
                    : $"- 血量：{FormatInteger(battle.PreBattleHealth)} → {FormatInteger(battle.PostBattleHealth)}（未知；异常增量已拒绝）")
            .AppendLine($"- 完美通关：{ClearName(battle.ClearStatus)}")
            .AppendLine("- 角色伤害：");
