    ...
    : IGrailCommandHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        stateHolder.Reset();
        // 1.2.95 备案清理：Reset 语义留痕——清了什么、何时清直接进事件日志，
        // 不再靠"买不到东西/祈愿计数跳变"的行为反推。（1.2.96 审查 A P3-1：
        // 挪到 Reset 之后=文案先于事实消除；枚举补全购买账本与 last-known 缓存。）
        executor.PublishTelemetry(
            "M8MatchBoundaryReset",
            "对局边界复位完成：执行器上场进度+购买账本、持有器已购集合/事件态/星徽账本/血与人口金币 last-known 缓存/阵容格数基线已统一清零。");
