    ...
    IPhase2RecognitionFeed? recognitionFeed = null) : IRewardStageAutomationController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 后续 CaptureForegroundAsync 只等"本局新帧"。
        lastFrameVersion = recognitionFeed?.LatestFrameVersion ?? 0;
        ...
        if (!await CloseShopAsync(windowHandle, "preparation_generic", cancellationToken))
        {
            return Failed(
                RewardStageAutomationStatus.RecoveryRequested,
                "未能确认 1-2 商店关闭。");
            ...
            CancellationToken cancellationToken)
    {
        var window = await foregroundGuard.WaitUntilForegroundAsync(
            windowHandle,
            cancellationToken);
        if (recognitionFeed is not null && recognitionFeed.IsAttached)
        {
            // 统一识别流：从 feed 取下一帧（记录员运行时 pipeline 约 100ms
            // 一帧），不再自己截图（用户架构：一个摄像机）。记录员未挂载/
            // 已停止（IsAttached=false）或等待超时时回退到自己的 capture。
            var frame = await recognitionFeed.WaitForNextFrameAsync(
                    lastFrameVersion,
                    TimeSpan.FromSeconds(2),
                    cancellationToken)
                .ConfigureAwait(false);
            lastFrameVersion = recognitionFeed.LatestFrameVersion;
        ...
        var fallback = await capture.CaptureAsync(
            window,
            cancellationToken);
    ...
    /// </summary>
    internal async Task<bool> BuyPopulationAsync(
        nint windowHandle,
        CancellationToken cancellationToken)
    {
        // StoreLevelValue(0.148,0.790,0.095,0.070) 归一点转 1920×1080 基准点，点两次购买经验。
        var buyPoint = new StandardPoint(
            (int)Math.Round(0.148 * 1920 + (0.095 / 2d) * 1920),
            (int)Math.Round(0.790 * 1080 + (0.070 / 2d) * 1080));
        var ok = true;
        for (var i = 0; i < 2 && ok; i++)   // 点两次购买经验
        {
            ok = await ClickStandardPointAsync(
                windowHandle,
                new StandardPoint(
                    (int)Math.Round(0.148 * 1920 + (0.095 / 2d) * 1920),
                    (int)Math.Round(0.790 * 1080 + (0.070 / 2d) * 1080)),
