...
public sealed class InvestmentStrategyPageReader(
    ...
    GameDataNameMatcher? matcher = null)
{
    private static readonly int[] CardCenters = [460, 925, 1390];
    private static readonly PixelRect ConfirmRegion = new(780, 955, 360, 70);
    private static readonly IReadOnlyList<PixelRect> NameRegions =
    [
        new(330, 445, 380, 80),
        new(795, 445, 380, 80),
    ...
    public async Task<IReadOnlyList<InvestmentStrategySlot>> ReadAsync(
        ...
        CancellationToken cancellationToken)
    {
        var selectedSlot = DetectSelectedSlot(frame);
    ...
    internal static int? DetectSelectedSlot(CaptureFrame frame)
    {
        if (MeanBrightness(frame, RewardRecognitionSupport.Scale(
                ConfirmRegion,
                frame)) < 100)
        {
            return null;
        ...
        for (var index = 0; index < CardCenters.Length; index++)
        {
            var center = CardCenters[index];
            var left = RewardRecognitionSupport.Scale(
                new PixelRect(center - 213, 215, 28, 700),
                frame);
            var right = RewardRecognitionSupport.Scale(
                new PixelRect(center + 185, 215, 28, 700),
                frame);
            if (MeanBrightness(frame, left) >= 85 &&
                MeanBrightness(frame, right) >= 85)
            {
                selected.Add(index);
            }
        }
    ...
    private static double MeanBrightness(CaptureFrame frame, PixelRect region)
    {
        var left = Math.Clamp(region.X, 0, frame.Width - 1);
        var top = Math.Clamp(region.Y, 0, frame.Height - 1);
        var right = Math.Clamp(region.X + region.Width, left + 1, frame.Width);
        var bottom = Math.Clamp(region.Y + region.Height, top + 1, frame.Height);
        long total = 0;
        long count = 0;
