...
public sealed partial class CurrencyWarsSituationScreenshotAnalyzer(
    ...
    InvestmentStrategyPageReader? investmentStrategyReader = null) :
    ISituationScreenshotAnalyzer
{
    ...
    private readonly InvestmentStrategyPageReader _investmentStrategyReader =
        investmentStrategyReader ?? new InvestmentStrategyPageReader(ocr, gameData);
    ...
    private readonly PreparationEconomyStabilizer _preparationEconomyStabilizer =
        new();
    ...
    public void SeedRunIdentity(string runId, RunIdentityEvidence identity)
    {
        ...
        if (identity.InvestmentStrategyIds.Count > 0)
        {
            _investmentStrategiesByRun[runId] =
                Observation<IReadOnlyList<string>>.Known(
                    identity.InvestmentStrategyIds,
                    1,
                    observedAt: DateTimeOffset.UtcNow);
        }
    }
    ...
    private static readonly PixelRect EconomyTightOcrRegion =
        ...
        // 问题4修复：左边界从 1628 扩到 1590——云游戏/2K 下两位数"11"
        // 的第一位可能落在 1628 左侧被切掉（用户实测 1↔11 跳变）。
        // 左扩后由 ReadIntegerAsync 的正则提取纯数字，图标/文字不参与。
        new(1590, 912, 105, 60);
    ...
    private readonly ConcurrentDictionary<
        string,
        Observation<IReadOnlyList<string>>> _investmentStrategiesByRun =
            new(StringComparer.Ordinal);
        ...
        int? backSlotCount = null)
    {
        ...
        // 2026-08-11 修复A：增量帧（incremental 带 KnownPageId）跳过全量
        // 页面分类（300-530ms）——画面静止页面未变，复用上次识别页面。
        // 此前增量帧也跑全量分类导致 worker 被占满（每 500ms 一个 ~500ms
        // 分析），识别间隔恶化到 6s，结算页只识别 1 帧、战斗结束确认失败。
        // 修复后增量帧只补失败字段，分类由 2s 全量兜底负责。
        var isIncrementalFrame = incremental is not null &&
                                 incremental.KnownPageId is not null;
        var classificationStopwatch = Stopwatch.StartNew();
        var classification = isIncrementalFrame
            ? null
            : pageClassifier.Classify(frame);
        classificationStopwatch.Stop();
        var afterClassification = overallStopwatch.Elapsed;
        var diagnosticInference = !isIncrementalFrame &&
            ...
            : null;
        var shouldProbeSettlementSemantics = !isIncrementalFrame &&
            (classification is null ||
        ...
        if (isIncrementalFrame)
        {
            page = Observation<string>.Known(
                ...
                [evidence with
                {
                    Locator = "incremental:reused-page",
                    Summary = $"增量帧复用上次页面 {incremental!.KnownPageId}（画面静止，跳过全量分类）。",
                    Confidence = 0.9
                }],
                frame.CapturedAt);
        }
        else if (semanticSettlement is not null)
        {
            page = Observation<string>.Known(
                semanticSettlement.PageId,
                semanticSettlement.Confidence,
        ...
        var economyWasSampled = false;
        // 2026-08-11 修复A：增量帧跳过 situation 页面分析器（economy/health/
        // storeLevel 快路径）——只交给 operational 增量补失败字段。
        if (!isIncrementalFrame &&
            classifiedPageId is
                "preparation_1_1" or "preparation_1_2" or "preparation_generic")
        {
            hasPreparationSnapshot = true;
            economyWasSampled = true;
        ...
        else if (!isIncrementalFrame && classifiedPageId is not null)
        {
            economyWasSampled = string.Equals(
                classifiedPageId,
                "reward_shop",
            ...
            pageSnapshotTask = classifiedPageId switch
            {
                ...
                "investment_environment" =>
                    AnalyzeInvestmentOptionsAsync(
                        ...
                        cancellationToken),
                "investment_strategy" =>
                    AnalyzeInvestmentStrategyAsync(
                        frame,
                        snapshot,
                        effectiveRunId,
                        evidence,
        ...
        if (operational is not null)
        {
            ...
            if (operational.PageFamily is
                    Phase2PageFamily.Preparation or Phase2PageFamily.Supply)
            {
                snapshot = snapshot with
                {
                    Economy = _preparationEconomyStabilizer.Observe(
                        effectiveRunId,
                        operational.NodeId,
                        snapshot.Economy,
                        economyWasSampled,
        ...
        if (snapshot.InvestmentStrategyIds.Status == ObservationStatus.Known &&
            snapshot.InvestmentStrategyIds.Value is { Count: > 0 })
        {
            _investmentStrategiesByRun[effectiveRunId] =
                snapshot.InvestmentStrategyIds;
        }
        else if (_investmentStrategiesByRun.TryGetValue(
                     effectiveRunId,
                     out var knownStrategies))
        {
            snapshot = snapshot with
            {
                InvestmentStrategyIds = knownStrategies
            };
        }
        ...
        bool includeFormation = true)
    {
        ...
        var economyTask = MeasureAsync(async () =>
        {
            ...
            var localized = await ReadPreparationEconomyAsync(
                    ...
                    cancellationToken)
                .ConfigureAwait(false);
            var hasCandidateConflict = localized.Uncertainty.Any(reason =>
                reason.Contains("冲突", StringComparison.Ordinal));
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 与 operational 层一致，用户要求）。本层（即时分析）无跨帧状态，
        // 直接以 Unknown 赋给本帧 snapshot.Health，不写入错误数据。
        var healthTask = Task.FromResult(
            Observation<int>.Unknown(
                "商店页不识别血量（商店 UI 遮挡）。",
                [evidence],
                frame.CapturedAt));
        var storeLevelTask = ReadStoreLevelAsync(
            frame,
            evidence,
            cancellationToken);
        await Task.WhenAll(slotsTask, economyTask, healthTask, storeLevelTask)
            .ConfigureAwait(false);
        ...
        return snapshot with
        {
            ...
            StoreLevel = storeLevelTask.Result,
            ShopCharacterIds = shop
        };
    }

    /// <summary>
    /// 备战页左下角"购买经验 Lv.N"等级识别（区域 StoreLevelValue，用户
    /// 2026-08-06 要求补录商店等级；此前该字段无识别代码，恒"未记录"）。
    /// </summary>
    private async Task<Observation<int>> ReadStoreLevelAsync(
        ...
        CancellationToken cancellationToken)
    {
        var value = await ReadIntegerAsync(
            frame,
            Phase2RecognitionRegions.StoreLevelValue.ToPixels(
                frame.Width,
                frame.Height),
            1,
            20,
        ...
        // 用户规则（2026-08-10）：商店 Lv 只可能是 3-10 级——识别到
        // 1 或 0 均视为 Lv.10（Lv.10 的十位 1 或个位 0 被误取）。
        return value is { Status: ObservationStatus.Known, Value: 0 or 1 }
            ? value with { Value = 10 }
            : value;
    }
    ...
    private async Task<RunSnapshot> AnalyzeInvestmentStrategyAsync(
        CaptureFrame frame,
        RunSnapshot snapshot,
        string runId,
        EvidenceReference evidence,
        CancellationToken cancellationToken)
    {
        var slots = await _investmentStrategyReader
            .ReadAsync(frame, cancellationToken)
            .ConfigureAwait(false);
        var selected = slots
            .Where(item => item.IsSelected && item.Strategy is not null)
            .ToArray();
        ...
        var item = selected[0];
        var strategies = Observation<IReadOnlyList<string>>.Known(
            [item.Strategy!.Id],
            item.Confidence,
            [evidence with
            {
                Locator = "ocr:investment-strategy-selected-card",
                Summary = $"唯一高亮卡片且确认按钮已启用：{item.Strategy.Name}"
            }],
            frame.CapturedAt);
        _investmentStrategiesByRun[runId] = strategies;
        return snapshot with { InvestmentStrategyIds = strategies };
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var templateResolution =
            PreparationEconomyCandidateResolver.ResolveTemplateCandidates(
                candidates.Select(item => new PreparationEconomyCandidate(
                    item.Source,
                    item.Value,
                    item.Confidence,
                    item.DigitCount)));
        if (templateResolution is not null)
        {
            return new Observation<int>
            {
                Status = ObservationStatus.Known,
                Value = templateResolution.Value,
                Confidence = templateResolution.Confidence,
                Evidence = attempts.Distinct().ToArray(),
                Uncertainty = templateResolution.Rule ==
                              PreparationEconomyCandidateResolver
        ...
        var distinctCandidateValues = candidates
            .Select(item => item.Value)
            .Distinct()
            .Count();
        var resolution =
            PreparationEconomyCandidateResolver.ResolveStrictMajority(
                candidates.Select(item => new PreparationEconomyCandidate(
                    item.Source,
                    item.Value,
                    item.Confidence,
        ...
        if (resolution is not null)
        {
            return new Observation<int>
            {
                ...
                Evidence = attempts.Distinct().ToArray(),
                Uncertainty = distinctCandidateValues > 1
