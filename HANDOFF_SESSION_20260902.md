...
- **1.2.87 补充（同日上午，用户确认方向+追加需求）**：①用户拍板：**AI 全程走软件通道操作游戏，不再用电脑操控**（computer-use 退役）；②追加 **SCREENSHOT 指令**——GdiGameCapture（GDI 屏幕 DC/PrintWindow）后台抓游戏窗口存 PNG 到 exe 同目录「指令测试-screen.png」（覆盖写），回执报路径+尺寸；纯只读、**决策层运行期间也允许**（监督 DECIDE 时看画面）、与 WGC 识别会话捕获单例零共享（每次 new 不冲突）、不抢用户前台；黑匣子 kind=screenshot。③**【发布剥离清单——最终交付版必须执行，2026-09-05 用户令】**：AI 辅助通道 KEY/CLICK/SCREENSHOT 三条指令对最终用户无用，真正发布给用户的版本必须全部去掉。当前设计已便于剥离：三条指令全部只存在于 CommandTestWindow.cs（测试台专类，普通模式不加载），不碰语义指令集/决策层/操作层；发布剥离时删 CommandTestWindow 中 KEY/CLICK/SCREENSHOT 三个 case+ExecuteRawInputAsync/ExecuteScreenshotAsync/TryParseInputKey+IInputController 注入即可（App.xaml.cs 的 DI 注册 IInputController 是产品在用的，保留）。**此清单在"最终发布"批次执行前每次 handoff 必须保持可见**。④构建 0/0（GdiGameCapture 非 IDisposable 教训：using 需核实接口）。
