...
- **🔴 聘用书选人专项素材确认（在屏,专项可写）**：视频 2 原片仍在桌面（`我做到了！A850 1-3 三星昔涟.mp4`,1920×1080 av1,已封箱）;重抽 94-101s+102-124s fps=2 密集帧（存 D:\CurrencyWarsData\sandbox-frames\hire-letter-96-99s\）分类扫描定出**三簇全貌**：簇1 90s=**令咒决议·行为限制（奖励=5费聘用书×2）vs 金色传说**;簇2 96-99s=绝地求生·进阶 vs 无限之釜;簇3 107-109.5s=令咒决议·奇迹代偿 vs 完美胜利。**交接里说的"回路过载"从未在屏,但同族聘用书来源试炼"行为限制"在屏=素材成立**——M3 决策器本就兼容两变体（OverloadKeyword=回路过载+RestrictKeyword=行为限制+奖励关键词"五费聘用书"精确排除四费）,引擎口径无缺口。wish_90s=聘用书侧祈愿帧（帧内右卡已金框选中态,静态帧不影响）;wish_119s=选人悬浮窗全帧。**TrialRecruit 静态帧边界（脚本编排依据）**：选人成功判定="悬浮窗退出后读不出候选名",静态帧下永不满足→3 次点卡→InputFailed→外层 OpenLettersAsync 6 次重试（每次=点备战席书格(281,951)→等600ms→OCR 4名框→匹配昔涟/Archer→点卡中心(623,272)）,obtained=2 恒 0 开→6 轮耗尽后 M3 照常回执（开书结果不进 M3 回执）——脚本按 bench/page 交替步序编排,重复点击在沙箱容差（200）内。
