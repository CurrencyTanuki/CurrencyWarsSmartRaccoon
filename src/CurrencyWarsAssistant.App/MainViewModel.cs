...
public sealed class MainViewModel : ObservableObject
{
    ...
    private bool _enableDebugEvidenceSaving;
    private bool _enablePerformanceMeasurement;
    private bool _enablePerformanceShadowValidation;
    private bool _enableHybridRealtimeRecognition;
    private bool _deleteScreenshotsAfterRunCompletion;
    private bool _enableRewardStageAutomation;
    private bool _enableEarlyStrongFormationPurchase = true;
    private bool _enableGalaxyScholarRewardStrategy;
    private bool _enablePresetPriorityDeployment = true;
    // 清单独立开关：默认值保持现有行为（命中即停、中途不执行、已有对局自动停）
    private bool _stopOnTargetedOpening;
    private bool _skipStartDuringActiveRun;
    private bool _autoStopOnExistingRun = true;
        ...
        IChallengeSummaryReportGenerator? summaryReportGenerator = null)
    {
        ...
        AutoRerollCommand = new AsyncRelayCommand(
            () => TrackActiveOperationAsync(
                () => RunFilterAsync(singleRound: false, useProfiles: false)),
            CanStart);
        RunProfilesCommand = new AsyncRelayCommand(
            () => TrackActiveOperationAsync(
                () => RunFilterAsync(singleRound: false, useProfiles: true)),
            CanStart);
        ...
        ClearAllLocalDataCommand = new AsyncRelayCommand(
            async () => await ClearAllLocalDataAsync(),
            () => !IsRunning && !IsPassiveCollectionRunning);
    ...
    public bool EnableDebugEvidenceSaving
    {
        get => _enableDebugEvidenceSaving;
        set
        {
            if (SetProperty(ref _enableDebugEvidenceSaving, value))
            {
                DebugEvidenceCollector.Enabled = value;
            }
        }
    }
    ...
    {
        get => _enablePerformanceMeasurement;
        set
        {
            _ = SetProperty(ref _enablePerformanceMeasurement, value);
            ApplyPerformanceDiagnosticsMode();
        }
    }

    public bool EnablePerformanceShadowValidation
    {
        get => _enablePerformanceShadowValidation;
        set
        {
            _ = SetProperty(ref _enablePerformanceShadowValidation, value);
            ApplyPerformanceDiagnosticsMode();
        }
    }

    public bool EnableHybridRealtimeRecognition
    {
        get => _enableHybridRealtimeRecognition;
        set => SetProperty(ref _enableHybridRealtimeRecognition, value);
    }

    private void ApplyPerformanceDiagnosticsMode() =>
        Phase2RealtimeDiagnostics.Mode = EnablePerformanceShadowValidation
            ? Phase2RealtimeDiagnosticsMode.ObserveOnly
            : EnablePerformanceMeasurement
                ? Phase2RealtimeDiagnosticsMode.MeasureOnly
                : Phase2RealtimeDiagnosticsMode.Legacy;
    ...
    /// <summary>
    /// 是否启用"预设优先上场角色"（默认开）。关闭时备战席前三个直接拖上前台。
    /// </summary>
    public bool EnablePresetPriorityDeployment
    {
        get => _enablePresetPriorityDeployment;
        set => SetProperty(ref _enablePresetPriorityDeployment, value);
    }

    /// <summary>清单开关1：刷到目标开局后是否停下（默认关=命中即停，不进行 1-2 干预）。</summary>
    public bool StopOnTargetedOpening
    {
        get => _stopOnTargetedOpening;
        set => SetProperty(ref _stopOnTargetedOpening, value);
    }

    /// <summary>清单开关2：对局中途点"开始刷开局"是否执行（默认关=检测到已有对局不执行）。</summary>
    public bool SkipStartDuringActiveRun
    {
        get => _skipStartDuringActiveRun;
        set => SetProperty(ref _skipStartDuringActiveRun, value);
    }

    /// <summary>清单开关3：主界面/启动检测到已有对局是否自动停下（默认开）。</summary>
    public bool AutoStopOnExistingRun
    {
        get => _autoStopOnExistingRun;
        set => SetProperty(ref _autoStopOnExistingRun, value);
    }
    ...
    public AsyncRelayCommand RunProfilesCommand { get; }
    ...
    public AsyncRelayCommand ClearAllLocalDataCommand { get; }
        ...
        try
        {
            ...
            void Apply()
            {
                ...
                foreach (var record in records)
                {
                    ...
                    var (damageLine, goldLine, theoryLine) = BuildChartLines(record.Nodes);
    ...
    /// <summary>
    /// 把存档节点的伤害/金币/理论出伤极限归一化成折线点串，
    /// 供历史对局窗口的"节点数值趋势"图使用（缺失值用上一有效值延续）。
    /// </summary>
    private static (string Damage, string Gold, string Theory) BuildChartLines(
        IReadOnlyList<CompletedRunNodeRecord> nodes)
    {
        ...
        var theory = nodes.Select(n =>
            TheoreticalDamageCalculator.HasActionLimit(n.NodeId)
                ? n.FinalBattle?.TheoreticalDamageLimit
                : null).ToArray();
        ...
        return (BuildLine(damage), BuildLine(gold), BuildLine(theory));
    }
    ...
    public async Task RefreshIncompleteRunsAsync()
    {
        try
        {
            ...
            void Apply()
            {
                ...
                OnPropertyChanged(nameof(RealtimeNodeEntries));
    ...
    /// <summary>
    /// 清除全部本地数据：对局记录（runs）、日志（logs）、用户配置（user-settings.json）。
    /// 删除后恢复代码默认值。此操作不可恢复。
    /// </summary>
    public async Task ClearAllLocalDataAsync()
    {
        ...
        try
        {
            var root = _runStore.RootDirectory;
            ...
            Status = "已清除全部本地数据，设置已恢复默认。";
            PublishApplicationWarning(
                "AllLocalDataCleared",
                "已清除全部本地数据（对局记录/日志/用户配置）。");
        }
        catch (Exception exception)
            when (exception is IOException or
                  UnauthorizedAccessException or
        ...
        {
            PublishApplicationWarning(
                "AllLocalDataClearFailed",
                $"清除本地数据失败：{exception.Message}");
        }
    }
    ...
    public void SaveUserSettings()
    {
        try
        {
            ...
            var settings = new UserSettingsSnapshot(
                ...
                EnableDebugEvidenceSaving: EnableDebugEvidenceSaving,
                EnablePerformanceMeasurement:
                    EnablePerformanceMeasurement,
                EnablePerformanceShadowValidation:
                    EnablePerformanceShadowValidation,
                EnableHybridRealtimeRecognition:
                    EnableHybridRealtimeRecognition,
                EnablePresetPriorityDeployment:
                    EnablePresetPriorityDeployment,
                StopOnTargetedOpening: StopOnTargetedOpening,
                SkipStartDuringActiveRun: SkipStartDuringActiveRun,
                AutoStopOnExistingRun: AutoStopOnExistingRun);
            File.WriteAllText(
                path,
                JsonSerializer.Serialize(
                    settings,
                    UserSettingsJsonOptions));
            Phase2RealtimeExecution.Mode = Phase2RealtimeExecution.ResolveMode(
                EnableHybridRealtimeRecognition);
            _eventSink.Publish(new TaskEvent(
                DateTimeOffset.Now,
                TaskEventLevel.Information,
    ...
    private void LoadUserSettings()
    {
        ...
        try
        {
            ...
            EnableDebugEvidenceSaving =
                settings.EnableDebugEvidenceSaving;
            EnablePerformanceMeasurement =
                settings.EnablePerformanceMeasurement;
            EnablePerformanceShadowValidation =
                settings.EnablePerformanceShadowValidation;
            EnableHybridRealtimeRecognition =
                settings.EnableHybridRealtimeRecognition;
            Phase2RealtimeExecution.Mode = Phase2RealtimeExecution.ResolveMode(
                EnableHybridRealtimeRecognition);
            DeleteScreenshotsAfterRunCompletion =
            ...
            EnablePresetPriorityDeployment =
                settings.EnablePresetPriorityDeployment;
            StopOnTargetedOpening = settings.StopOnTargetedOpening;
            SkipStartDuringActiveRun = settings.SkipStartDuringActiveRun;
            AutoStopOnExistingRun = settings.AutoStopOnExistingRun;
    ...
    private async Task RunFilterAsync(
        ...
        bool useProfiles)
    {
        // 开始刷开局前，强制重新识别/定位游戏窗口：用户重开游戏后，
        // 旧的窗口句柄会失效（本会话就因此出现 WindowUnavailable）。
        // RefreshWindows 会重新枚举并将 SelectedWindow 匹配到最新句柄。
        RefreshWindows();
        ...
        if (useProfiles && !RerollProfiles.Any(item => item.IsEnabled))
        {
            Status = "未启用任何刷取方案；请先在下方勾选至少一个刷取方案再开始。";
            _eventSink.Publish(new TaskEvent(
                DateTimeOffset.Now,
                TaskEventLevel.Warning,
                "ProfilesRunGuard",
        ...
        IsRunning = true;
        // 自动刷开局的首个目标位于游戏右上角；操作面板若不穿透会覆盖该点。
        ...
        try
        {
            ...
            var filters = BuildFilterSet(useProfiles);
            _eventSink.Publish(new TaskEvent(
                DateTimeOffset.Now,
                TaskEventLevel.Information,
                "FilterConfiguration",
            ...
            var configuration = Phase1RunConfiguration.Create(
                ...
                new OpeningRerollLoopOptions
                {
                    ...
                    MaximumRuntime = singleRound
                        ...
                        : null,
                    DeployMatchedOpening =
                        !singleRound && !StopOnTargetedOpening,
                    CompleteRewardStages =
                        !singleRound && (HasConfiguredStrategy(useProfiles) ||
                            EnableRewardStageAutomation),
                    ...
                    EnablePresetPriorityDeployment =
                        EnablePresetPriorityDeployment,
                    IgnoreActiveRun =
                        !AutoStopOnExistingRun && SkipStartDuringActiveRun
                });
    ...
    /// <summary>
    /// 血量Δ显示（用户 2026-08-05 规则）：完美通关但血量未识别时登记 +2
    /// （完美判定本就包含 healthDelta==2 或 postBattleHealth==100；血量未知
    /// 但行动充足判完美时，回血量视为 +2）。
    /// </summary>
    internal static string FormatHealthDeltaDisplay(
        HistoricalNodeDashboardEntry node)
    {
        return node.HealthDelta switch
        {
            > 0 => $"+{node.HealthDelta}",
            0 => "±0",
            < 0 => node.HealthDelta.Value.ToString(),
            _ when node.HealthDepleted => "↓?",
            // 3-7 整局评级页不显示血量Δ（用户 2026-08-06 规则）：
            // 行动值 ≥100 → +42；行动值 <100 → +2（与常规完美节点一致）。
            _ when node.ClearStatus == NodeClearStatus.Perfect &&
                   string.Equals(
                       node.NodeId,
                       "3-7",
                       StringComparison.OrdinalIgnoreCase) &&
                   (node.RemainingActionValue ?? 0) >= 100 => "+42",
            _ when node.ClearStatus == NodeClearStatus.Perfect => "+2",
            _ => "—"
        };
    }
    ...
    private void ApplyHistoricalDashboard(HistoricalDashboardSnapshot snapshot)
    {
        ...
        for (var index = 0; index < snapshot.Nodes.Count; index++)
        {
            ...
            DashboardRows.Add(BuildDashboardRow(
                node,
                damageValues,
        ...
        DashboardSummaryText = $"已记录 {snapshot.Nodes.Count} 个节点 · 最近 {latest.NodeId}";
    }

    private static HistoricalDashboardRow BuildDashboardRow(
        HistoricalNodeDashboardEntry node,
        IReadOnlyList<long?> damageValues,
        HistoricalDamageScale damageScale,
        bool isLatest)
    {
        return new HistoricalDashboardRow(
            node.NodeId,
            ...
            node.HealthDelta switch
            {
                ...
                _ when node.ClearStatus == NodeClearStatus.Perfect => "Positive",
                _ => "Unknown"
            },
            ...
            TheoreticalDamageCalculator.HasActionLimit(node.NodeId)
    ...
    /// <summary>
    /// 构建刷开局筛选集。useProfiles=true 时只按"刷取方案"(底部)执行，
    /// 顶部全局环境/阵营/词条一律不携带；useProfiles=false 时只按顶部全局
    /// 配置执行，不携带刷取方案。两者完全隔离、互不影响。
    /// </summary>
    /// <summary>
    /// 当前生效配置里是否已设置投资策略：用上方全局按钮(useProfiles=false)看
    /// 主界面策略筛选；用下方刷取方案按钮(useProfiles=true)看启用的刷取方案。
    /// 与"上下配置隔离、互不影响"的设定保持一致：已配策略则命中后必打奖励关去刷它。
    /// </summary>
    private bool HasConfiguredStrategy(bool useProfiles) =>
        useProfiles
            ? RerollProfiles.Any(profile =>
                profile.IsEnabled &&
                profile.PreferredInvestmentStrategyIds.Count > 0)
            : InvestmentStrategyFilters.Items.Any(item =>
                item.SelectionMode == AppFilterSelectionMode.Required);

    private OpeningFilterSet BuildFilterSet(bool useProfiles) =>
        new()
        {
            InvestmentEnvironments = useProfiles
                ? []
                : BuildItemFilters(InvestmentEnvironmentFilters.Items),
            Competitors = useProfiles
                ? []
                : BuildItemFilters(CompetitorFilters.Items),
            EnemyModifiers = useProfiles
                ? []
                : BuildItemFilters(EnemyAffixFilters.Items),
            Profiles = useProfiles
                ? RerollProfiles
                    .Select(BuildProfileFilter)
                    .ToArray()
                : [],
            Combinations = useProfiles
                ? []
                : SpecialCombinations
                    .DistinctBy(
                        item => item.Id,
    ...
    private void NotifyCommands()
    {
        ...
        RunProfilesCommand.NotifyCanExecuteChanged();
    ...
    private sealed record UserSettingsSnapshot(
        ...
        bool EnableDebugEvidenceSaving = false,
        bool EnablePerformanceMeasurement = false,
        bool EnablePerformanceShadowValidation = false,
        bool EnableHybridRealtimeRecognition = false,
        bool EnablePresetPriorityDeployment = true,
        bool StopOnTargetedOpening = false,
        bool SkipStartDuringActiveRun = false,
        bool AutoStopOnExistingRun = true);
