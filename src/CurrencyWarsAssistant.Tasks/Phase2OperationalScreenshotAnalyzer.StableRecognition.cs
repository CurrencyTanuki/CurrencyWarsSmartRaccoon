...
public sealed partial class Phase2OperationalScreenshotAnalyzer
{
    ...
    internal static string? PreparationNodeFromPageId(string? pageId)    {
        ...
        if (plane is < 1 or > 3 || node is < 1 or > 9)
        {
            return null;
        }

        // 用户 2026-08-11：补给节点 1-5、2-3、3-3 只有页面没有备战/战斗，
        // 正常流程绝不会出现在备战识别里——识别到它们说明节点号读错，
        // 必须返回无效让上层重新识别，不得落盘污染对局归档。
        if ((plane == 1 && node == 5) ||
            (plane == 2 && node == 3) ||
            (plane == 3 && node == 3))
        {
    ...
    /// <summary>
    /// The expanded reward-shop overlay covers the stable preparation HUD.
    /// Reuse the last preparation evidence without sampling covered pixels or
    /// advancing retry/signature state. Shop-only fields are read by the
    /// dedicated shop analyzer; returning to preparation performs a normal scan.
    /// </summary>
    private StableRecognitionPlan ReuseStableRecognitionsForCoveredOverlay(
        string runId)
    {
        var run = GetStableRun(runId);
        lock (run.Gate)
        {
            run.LastAccessedAt = DateTimeOffset.UtcNow;
            return new StableRecognitionPlan(
                RecognizeNegativeAffixes: false,
                RecognizeEnvironment: false,
                RecognizeStrategies: false,
                AffixSignature: 0,
