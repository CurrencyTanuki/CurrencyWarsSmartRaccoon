...
public sealed class ChallengeSummaryReportTests
{
    ...
    public async Task FullReportContainsEffectiveFieldsAndIsIdempotent()
    {
        ...
        try
        {
            ...
            var generator = new ChallengeSummaryReportGenerator(catalog);
            ...
            var markdown = await File.ReadAllTextAsync(Path.Combine(
                root,
                "reports",
            ...
            var theoryIndex = html.IndexOf(
                "<small>阵容理论出伤极限</small>",
                StringComparison.Ordinal);
            var goldIndex = html.IndexOf(
                "<small>金币</small>",
                StringComparison.Ordinal);
            Assert.True(theoryIndex >= 0 && goldIndex > theoryIndex);
            Assert.Contains("1.8亿", html);
            Assert.Contains("180,000,000", html);
            Assert.Contains("character-alpha", html);
            Assert.Contains("2费", html);
            Assert.Contains("2费", markdown);
            Assert.Contains("equipment-alpha", html);
            Assert.DoesNotContain("奖励金币", html, StringComparison.Ordinal);
            Assert.DoesNotContain("<th>奖励</th>", html, StringComparison.Ordinal);
            Assert.DoesNotContain("奖励金币", markdown, StringComparison.Ordinal);
            ...
            Assert.DoesNotContain("专家顾问", html);
    ...
    [Fact]
    public async Task ReportDamageChartUsesNodeIdRewardRuleForScale()
    {
        // 问题回归（2026-08-05 用户实测）：HTML 报告与操作面板的对数自动
        // 判定必须用节点号规则（1-8/2-6/3-6），不能用 IsRewardNode 字段
        // （页面类型 reward_battle/battle_generic，实机几乎全 True → 过滤
        // 后只剩 1 个正数 → 强制线性，伤害图被拉平）。
        // 本用例：2-6 是奖励关（节点号规则），即使它的伤害极大（500 亿），
        // 也必须被排除出"有效最高伤害"（hero 区）。
        var root = TemporaryDirectory();
        try
        {
            var baseline = CompleteRun();
            // 把 2-1 替换为 2-6（奖励关），FinalBattle 伤害极大
            var run = baseline with
            {
                Nodes = baseline.Nodes.Select(item => item.NodeId == "2-1"
                    ? item with
                    {
                        NodeId = "2-6",
                        FinalBattle = item.FinalBattle! with
                        {
                            SelectedDamage = 50_000_000_000,
                            TotalDamage = 50_000_000_000,
                            TheoreticalDamageLimit = 60_000_000_000
                        }
                    }
                    : item).ToArray()
            };
            ...
            // 2-6 是奖励关（节点号规则）：不得作为 hero 区"有效最高伤害"
            //（旧逻辑用 IsRewardNode 字段，若该字段未标 true 会把 2-6 计入）
            Assert.DoesNotContain(
                "有效最高伤害</span><b class=\"gold\">500亿",
                html,
                StringComparison.Ordinal);
            // 图线判定仍正常（非奖励节点 1-1/1-3/3-1 跨度 1800 倍 → 对数）
            Assert.Contains("对数尺度", html, StringComparison.Ordinal);
        }
        finally
        {
            Directory.Delete(root, recursive: true);
    ...
    [Fact]
    public async Task ReportHidesLegacyNoActionTheoryAndImplausibleHealthGain()
    {
        var root = TemporaryDirectory();
        try
        {
            ...
            var run = baseline with
            {
                Nodes = baseline.Nodes.Select(item => item.NodeId switch
                {
                    "1-1" => item with
                    {
                        FinalBattle = item.FinalBattle! with
                        {
                            TheoreticalDamageLimit = 987_654_321,
                            BaseMaximumActionValue = 180,
                            EffectiveMaximumActionValue = 180,
                            TheoreticalDamageRule = "legacy D/U*M"
                        }
                    },
                    "2-1" => item with
                    {
            ...
            var htmlPath = await new ChallengeSummaryReportGenerator()
                .GenerateAsync(root, run, CancellationToken.None);
            var markdownPath = Directory.EnumerateFiles(
                    Path.Combine(root, "reports"),
                    "challenge-summary.md")
                .Single();
            ...
            Assert.DoesNotContain("987,654,321", html, StringComparison.Ordinal);
            Assert.DoesNotContain("987,654,321", markdown, StringComparison.Ordinal);
            Assert.Contains("不适用（该节点没有行动值上限）", html);
            Assert.Contains("不适用（该节点没有行动值上限）", markdown);
            Assert.DoesNotContain("+62", html, StringComparison.Ordinal);
            Assert.DoesNotContain("+62", markdown, StringComparison.Ordinal);
            Assert.Contains("异常增量已拒绝", html);
            Assert.Contains("异常增量已拒绝", markdown);
        }
        finally
        {
            Directory.Delete(root, recursive: true);
