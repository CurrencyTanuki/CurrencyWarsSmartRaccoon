...
public static class FateGrailFormationPolicy
{
    ...
    // 2026-08-26 用户裁决：命杯成员要"越多越好"。祈愿试炼弹框已全局适配(WishTrialSelectionAutomation)，
    // 不再因"避免触发试炼弹框"而限制命杯上场数量。设为实际可上场上限(9)即等效"不限"。
    public const int MaximumDeployedCharacters = 9;
...
public sealed class InitialRewardFormationPlanner
{
        ...
        int deployedFateGrailCount)
    {
        var fateGrailCount = deployedFateGrailCount + selected.Count(item =>
            FateGrailFormationPolicy.IsCandidate(item.Character));
        // 2026-08-26 用户明确"命杯越多越好"：命杯成员优先上场。
...
public sealed class PreparationBoardOptions
{
    ...
    /// <summary>
    /// 投资环境为「英雄登场」时，备战席第一号位（最左槽位，BenchSlot==0）
    /// 的角色是必须保留的关键角色，本局所有自动卖出都不得触碰它。
    /// </summary>
    public bool ProtectFirstBenchSlotFromSale { get; init; }
    /// <summary>
    /// 是否启用"预设优先上场角色"：开启(default)按预设输出角色名单优先上场，
    /// 命中后不再补其它角色；关闭时直接把备战席前三条(槽0/1/2)拖到前台，
    /// 不做名单筛选，后续金矿/买卖/出战流程不变。
    /// </summary>
...
public sealed class PreparationBenchSalePlanner
{
        ...
        IEnumerable<string>? deployedCharacterNames = null)
    {
        ...
        // 投资环境「英雄登场」时：备战席第一号位（最左，BenchSlot==0）的角色
        // 是必须保留的关键角色，禁止自动卖出。
        var heroEntranceProtectedBenchSlots = options.ProtectFirstBenchSlotFromSale
            ? availableBench
                .Where(item => item.BenchSlot == 0)
                .Select(item => item.BenchSlot)
                .ToArray()
            : [];
        var reservedBenchSlots = customRetainedBenchSlots
            .Concat(automaticallyRetainedBenchSlots)
            .Concat(heroEntranceProtectedBenchSlots)
            .ToHashSet();
    ...
    IPreparationBoardCompletionController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var bench = await ReadStableBenchAsync(
            ...
            expectedPreparationPageId: "preparation_generic",
            cancellationToken);
        if (bench is null)
        {
            return Result(
        ...
        // 关闭"预设优先上场角色"：不做名单筛选/优先级规划，直接把备战席
        // 前三个槽位(0/1/2)拖到前台，后续 1-1 商店补员/出售/出战流程不变。
        if (!options.EnablePresetPriorityDeployment)
        {
            return await PrepareTopThreeWithoutPresetAsync(
                windowHandle,
                bench,
                cancellationToken);
        }
    ...
    /// <summary>
    /// 关闭"预设优先上场角色"时的一号位部署：不做名单筛选/优先级规划，
    /// 直接把备战席前三个槽位(0/1/2)的角色拖到前台 0/1/2 号位。
    /// 保留真实备战席识别结果，供后续 1-1 商店补员/出售/出战使用。
    /// </summary>
    private async Task<PreparationBoardResult> PrepareTopThreeWithoutPresetAsync(
        nint windowHandle,
        IReadOnlyList<RecognizedBenchCharacter> bench,
        CancellationToken cancellationToken)
    {
        ...
        var ordered = bench
            .OrderBy(item => item.BenchSlot)
            .Take(Math.Min(3, BenchSlots.Count))
            .ToArray();
        if (ordered.Length == 0)
        {
            return Result(
                ...
                bench,
                [],
                "关闭预设优先上场后：备战席未识别到可上场角色；本轮按不可用处理。");
        }

        for (var index = 0; index < ordered.Length; index++)
        {
            var item = ordered[index];
            var placement = new PreparationPlacement(
                item,
                PreparationLane.Front,
                index);
            var deployed = await DeployWithVerificationAsync(
            ...
            if (!deployed)
            {
                return Result(
                    ...
                    bench,
                    placements,
                    $"关闭预设优先上场后台未确认“{item.Character.Name}”进入前台" +
                    $"{index + 1}号位；已停止后续拖拽。");
            }
        ...
        Publish(
            TaskEventLevel.Information,
            "PreparationTopThreeDeployed",
            "未启用预设优先上场：备战席前三个角色已拖到前台 1/2/3 号位。");
        return Result(
            PreparationBoardStatus.Deployed,
            bench,
            placements,
            $"未启用预设优先上场：已把备战席前 {placements.Count} 个角色拖到前台。");
    }
            ...
            CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= 8; attempt++)
        {
            ...
            if (page?.PageId is string actualPageId &&
                (string.Equals(
                     actualPageId,
                     expectedPreparationPageId,
                     StringComparison.OrdinalIgnoreCase) ||
                 actualPageId.StartsWith(
                     "preparation_",
                     StringComparison.OrdinalIgnoreCase)))
            {
