...
public sealed class HistoricalDashboardProjectionTests
{
    ...
    [Fact]
    public void DamageScaleUsesNodeIdRewardRuleNotPageTypeField()
    {
        // 问题回归（2026-08-05 用户实测：最终伤害图被拉平、自动判定为线性）：
        // 奖励节点判定必须用节点号规则（1-8/2-6/3-6），不能用 IsRewardNode
        // 字段（页面类型 reward_battle/battle_generic = 1-1/1-2 奖励关页面，
        // 实机会把几乎全部节点标记为奖励 → 过滤后只剩 1 个正数值 →
        // SelectDamageScale 强制线性）。
        // 模拟实机 run-20260805-023158：伤害跨度 64000~49.6 亿（ratio≈77565），
        // 各节点 IsRewardNode 字段 = true（页面类型），但节点号均非奖励关。
        var projection = new HistoricalDashboardProjection();
        var nodes = new[]
        {
            ("1-1", 64_000L),
        ...
        foreach (var (nodeId, damage) in nodes)
        {
            projection.Observe("run-scale", Preparation("run-scale", nodeId, 10, 0));
            var battle = new FinalNodeBattleState(
                nodeId,
                [],
                damage,
                RemainingActionValueState.Create(1, 50),
                DateTimeOffset.Parse("2026-07-30T23:00:00+08:00"),
                Evidence($"final-{nodeId}"),
                SelectedDamage: damage,
                SelectedDamageSource: FinalDamageSelectionSource.BattleLastFrame,
                IsRewardNode: true, // 页面类型字段标记为奖励（旧逻辑的误判来源）
                TheoreticalDamageLimit: damage * 15 / 10); // 理论极限同样大跨度
            projection.Observe(
                "run-scale",
                Analysis(
                    "run-scale",
        ...
        var snapshot = projection.Current;
        // 节点号均非 1-8/2-6/3-6 → 全部参与尺度判定 → 跨度 77565 倍 → 对数
        Assert.Equal(HistoricalDamageScale.Logarithmic, snapshot.DamageScale);
        Assert.Equal(HistoricalDamageScale.Logarithmic, snapshot.TheoryScale);
    }

    [Fact]
    public void DamageScaleStaysLinearWhenOnlyRewardNodesHaveWideRange()
    {
        // 只有当节点号规则的奖励节点（1-8/2-6/3-6）覆盖大跨度时，
        // 剩余非奖励节点跨度小 → 保持线性（奖励节点不参与 Y 轴范围）。
        var projection = new HistoricalDashboardProjection();
        foreach (var (nodeId, damage) in new[]
                 {
                     ("1-1", 100_000L),
                     ("1-2", 120_000L),
                     ("1-3", 150_000L),
                     ("2-6", 5_000_000_000L) // 节点号规则的奖励关（跨度大但排除）
                 })
        {
            projection.Observe("run-linear", Preparation("run-linear", nodeId, 10, 0));
            var battle = new FinalNodeBattleState(
                nodeId,
                [],
                damage,
        ...
        var snapshot = projection.Current;
        Assert.Equal(HistoricalDamageScale.Linear, snapshot.DamageScale);
    }
    ...
    private static ScreenshotAnalysisResult WithPreparationScalars(
        ...
        double confidence)
    {
        var observedAt = analysis.Snapshot.AsOf;
        var economy = Observation<int>.Known(gold, confidence, observedAt: observedAt);
        var healthObservation = Observation<int>.Known(
            health,
            confidence,
            observedAt: observedAt);
        var store = Observation<int>.Known(
            storeLevel,
            confidence,
            observedAt: observedAt);
    ...
    [Fact]
    public void Observe_RewardShopFrameDoesNotPolluteDetailedPreparationValues()
    {
        var projection = new HistoricalDashboardProjection();
        var preparation = WithPreparationScalars(
            Preparation("run-detail", "1-1", 1, 0),
            gold: 1,
            health: 68,
            storeLevel: 5,
            confidence: 0.95);
        projection.Observe("run-detail", preparation);

        var rewardShop = WithPreparationScalars(
            Analysis(
                "run-detail",
                "1-1",
                Phase2PageFamily.Preparation,
            ...
            storeLevel: 7,
            confidence: 0.95);
        projection.Observe("run-detail", rewardShop);

        var detail = Assert.Single(projection.Current.DetailNodes);
        Assert.Equal(1, detail.LatestSnapshot!.Economy.Value);
        Assert.Equal(68, detail.LatestSnapshot.Health.Value);
        Assert.Equal(5, detail.LatestSnapshot.StoreLevel.Value);
    }

    [Fact]
    public void Observe_LowConfidencePreparationScalarsRequireCrossFrameAgreement()
    {
        var projection = new HistoricalDashboardProjection();
        var first = WithPreparationScalars(
            Preparation("run-fusion", "1-1", 1, 0),
            gold: 1,
            health: 68,
            storeLevel: 5,
            confidence: 0.65);

        projection.Observe("run-fusion", first);

        var awaiting = Assert.Single(projection.Current.DetailNodes).LatestSnapshot!;
        Assert.Equal(ObservationStatus.Unknown, awaiting.Economy.Status);
        Assert.Equal(ObservationStatus.Unknown, awaiting.Health.Status);
        Assert.Equal(ObservationStatus.Unknown, awaiting.StoreLevel.Status);

        projection.Observe("run-fusion", first);

        var stable = Assert.Single(projection.Current.DetailNodes).LatestSnapshot!;
        Assert.Equal(1, stable.Economy.Value);
        Assert.Equal(68, stable.Health.Value);
        Assert.Equal(5, stable.StoreLevel.Value);

        var oneBadFrame = WithPreparationScalars(
            Preparation("run-fusion", "1-1", 12, 0),
            gold: 12,
            health: 0,
            storeLevel: 7,
            confidence: 0.65);
        projection.Observe("run-fusion", oneBadFrame);

        var afterBadFrame = Assert.Single(projection.Current.DetailNodes).LatestSnapshot!;
        Assert.Equal(1, afterBadFrame.Economy.Value);
        Assert.Equal(68, afterBadFrame.Health.Value);
        Assert.Equal(5, afterBadFrame.StoreLevel.Value);
    }
}
