...
public sealed class Win32InputControllerTests
{
    ...
    [Fact]
    public async Task ModifierClickDoesNotPressWhenAnotherWindowCoversTarget()
    {
        var window = Window();
        var backend = new RecordingBackend(window.Handle)
        {
            WindowAtPointHandle = 999
        };
        var controller = Controller(window, backend);

        var result = await controller.ClickWithModifierAsync(
            Target(window),
            InputKey.LeftAlt,
            new ActionPolicy { AfterActionDelay = TimeSpan.Zero },
            CancellationToken.None);
        ...
        Assert.False(result.Succeeded);
        Assert.Contains("window-at-point", backend.Events);
        Assert.DoesNotContain("down", backend.Events);
        Assert.DoesNotContain("up", backend.Events);
        Assert.Equal(
            [
    ...
    [Fact]
    public async Task ModifierClickAllowsChildWindowOwnedByGameAtTarget()
    {
        var window = Window();
        var backend = new RecordingBackend(window.Handle)
        {
            WindowAtPointHandle = 789,
            WindowAtPointRootHandle = window.Handle
        };
        var controller = Controller(window, backend);
    ...
    private sealed class RecordingBackend : IWin32InputBackend
    {
        ...
        public nint? WindowAtPointHandle { get; init; }
        public nint? WindowAtPointRootHandle { get; init; }
        ...
        {
            Events.Add(keyUp ? "key-up" : "key-down");
            return 1;
        }
        ...
        public nint WindowFromPoint(PixelPoint screenPoint)
        {
            ...
            return WindowAtPointHandle ?? _gameWindow;
        }

        public nint GetRootWindow(nint windowHandle)
        {
            Events.Add("root-window");
            return windowHandle == WindowAtPointHandle
                ? WindowAtPointRootHandle ?? windowHandle
                : _gameWindow;
        }
    }
