...
public sealed partial class OpenCvUiDigitSequenceRecognizer
{
    ...
    // 16/20 px windows match partial strokes inside ordinary 40 px digits. Keep them
    // opt-in for the StoreLevel "Lv." probe, whose real trailing digit can be 16 px.
    private static readonly IReadOnlyList<int> SlidingTemplateHeights =
        [24, 28, 32, 36, 40, 48, 56, 64];
    private static readonly IReadOnlyList<int> SlidingTemplateHeightsWithSmallGlyphs =
    ...
    public UiDigitSequenceRecognition Recognize(
        ...
        bool includeSmallSlidingGlyphs = false)
    {
    ...
    private UiDigitSequenceRecognition RecognizeBySlidingTemplates(
        ...
        int minimumValue,
        int maximumValue,
        bool includeSmallGlyphs)
    {
        var candidates = new List<SlidingDigitCandidate>();
        var templateHeights = includeSmallGlyphs
            ? SlidingTemplateHeightsWithSmallGlyphs
            : SlidingTemplateHeights;
        foreach (var (template, digit) in templates)
        {
            ...
            foreach (var height in templateHeights)
            {
                var width = Math.Max(
                    8,
