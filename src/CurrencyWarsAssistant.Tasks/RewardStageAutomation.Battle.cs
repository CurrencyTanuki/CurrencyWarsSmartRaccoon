...
public sealed partial class RewardStageAutomationController
{
    ...
    /// <summary>
    /// 「1-3 三星五费」执行层接入点：在投资策略页按偏好 id 挑选并确认一张策略
    ///（含三连刷兜底），找不到则返回 InvestmentStrategyNotFound（上层据此重刷）。
    /// 这是 private <see cref="SelectInvestmentStrategyAsync"/> 的 internal 包装，
    /// 供 <see cref="FateGrailProductionExecutor"/> 复用（买二极管276 等）。
    /// </summary>
    internal Task<RewardStageAutomationResult> TrySelectInvestmentStrategyAsync(
        nint windowHandle,
        IReadOnlySet<string> preferredStrategyIds,
        CancellationToken cancellationToken) =>
        SelectInvestmentStrategyAsync(
